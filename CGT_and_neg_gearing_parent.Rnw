...
cumsum_reset <- function(x) {
  x * ave(x, c(0, cumsum(diff(x) != 0)), FUN = seq_along)
}
...
hilda_survo <- survfit(Surv(optimistic_avg_spell, censor_status) ~ 1, data = hilda_surv_data_weights, weights = lon_weight)
hilda_survp <- survfit(Surv(pessimistic_avg_spell, censor_status) ~ 1, data = hilda_surv_data_weights, weights = lon_weight)

bind_rows(
  "high" = data_frame(years = hilda_survp$time, survival = hilda_survp$surv, survival_upper = hilda_survp$upper, survival_lower = hilda_survp$lower),
   "low" = data_frame(years = hilda_survo$time, survival = hilda_survo$surv, survival_upper = hilda_survo$upper, survival_lower = hilda_survo$lower), 
   "low" = data_frame(years = 0, survival = 1, survival_upper = 1, survival_lower = 1), 
   "high" = data_frame(years = 0, survival = 1, survival_upper = 1, survival_lower = 1), 
  .id = "imputer"
) %>% 
  grplot(aes(x = years, y = survival, ymin = survival_lower, ymax = survival_upper, colour = imputer, group = imputer)) + 
  geom_line() +
...
data.table::CJ(
  ...
  filter(!(Indexed & CGT_discount != 0.00)) %>%  # if indexed, CGT is ignored, so just choose keep one CGT discount
  rowwise %>%
  mutate(
    emtr = emtr_rental(return_type = return_type, 
  ...
  arrange(desc(capital_gain_return)) %>%
  # cosmetics
  mutate(capital_gain_return = paste0("Nominal annual capital gain__", formatC(100*capital_gain_return, flag = "#", format = "f", digits = 1), percentage_symbol = "\\%")) %>%
  spread(capital_gain_return, emtr) %>% 
  ungroup %>%
  arrange(marginal_tax_rate, desc(return_type), `Nominal annual capital gain__5.0\\%`) %>%
  mutate(`CGT treatment` = if_else(Indexed, "Indexed", paste(latex_percent(CGT_discount, digits = 0, percentage_symbol = "\\%"), "discount"))) %>%
  select(-Indexed, -CGT_discount) %T>%
  {grattanCharts:::chart_data(.)} %>%
  mutate_each(funs(digit1_01_TeX)) %>%
  mutate_each(funs(omit_repeated)) %>%
  setnames(old = grep("_[a-z]", names(.), value = TRUE), 
           new = sapply(gsub("_([a-z])", " \\1", grep("_[a-z]", names(.), value = TRUE)), capitalize)) %>% .[] %>%
  ...
  mutate(`\\phantom{.}` = if_else(!is.na(`Marginal tax rate`) & `Marginal tax rate` != first(`Marginal tax rate`), 
                                 "&\\phantom{.}\\\\[-6pt]", 
...
EMTR_Appendix_Table %>%
  mutate_each(funs(shade_cell), -`Marginal tax rate`, -`Return type`, -`CGT treatment`, -`\\phantom{.}`) %>%
  {suppressWarnings(print_2heading_xtable(., 
                                          sanitize.text.function = function(x) x, 
                                          tabular.environment = "tabular", 
                                          xtable.align = "lQlllrrrrr",
