...
TARGET.FY <- "2017-18"
use.sample.fy <- "2015-16"  # deprecated
latest.sample.fy <- "2015-16"

# HILDA
hilda_year <- 2017L
wave <- letters[hilda_year - 2000L]


on_travis <- nzchar(Sys.getenv("TRAVIS")) && identical(Sys.getenv("TRAVIS"), "true")
message("\ton_travis: ", on_travis)
warning("ABS Housing finance data (Cat 5609.0) is out of date by more than a quarter.")
...
# HILDA takes 7 minutes to load and tidy
reload_all_data <- TRUE
cache_most <- FALSE #&& !reload_all_data
@

\rehead[\normalfont\texttt{new.discount} = \Sexpr{new.discount}\quad
        \texttt{TARGET.FY} = \Sexpr{TARGET.FY}\quad
        \texttt{latest.sample.fy} = \Sexpr{latest.sample.fy}]%
        {\normalfont\texttt{new.discount} = \Sexpr{new.discount}\quad
        \texttt{TARGET.FY} = \Sexpr{TARGET.FY}\quad
        \texttt{latest.sample.fy} = \Sexpr{latest.sample.fy}}
\rohead[\normalfont\texttt{new.discount} = \Sexpr{new.discount}\quad
...
<<knitrOpts, echo=FALSE, results='hide', include=FALSE>>=
library(knitr)
...
setup_knitr_for_grattex( "CGT_and_neg_gearing_parent-CHUNKTIMINGS.txt")
options(xtable.sanitize.text.function = function(x) {
  o <- x
  if (is.character(x)) {
    o <- gsub("(?<!(\\\\))[%]", "\\\\%", o, perl = TRUE)
    o <- gsub("(?<!(\\\\))[$]", "\\\\$", o, perl = TRUE)
    o <- gsub("(?<!(\\\\))[_]", "\\\\_", o, perl = TRUE)
  }
  o
},
...
if (is_knitting()) {
  ...
  invisible(file.create("memory-usage.txt"))
}
...
library(testthat)
library(hutilscpp)  # hughparsonage/hutilscpp
library(haven)
library(knitr)
library(data.table)
options("datatable.integer64" = "double")  # Reading billions remain numeric
library(dplyr)
library(gridExtra)
library(ggplot2)
library(scales)
...
<<EarlySessionInfo>>=
# Needed here to avoid a memory error on travis if put later
library(sessioninfo)
...
<<downloadGrattex, include=FALSE>>=
overwrite_grattex <- 
  OR(!file.exists("grattan.cls"),
     (difftime(Sys.time(), file.info("grattan.cls")$ctime, units = "days") > 10))
...
latex_dollar <- function (x, digits = 0){
    ...
    if_else(x < 0, paste0("$-$", "\\$", commaz), paste0("\\$", commaz))
}
@
...
boldxt <- function(x, sanitize = TRUE) {
  # Replace $ without \\
  x <- if_else(grepl("((?<!\\\\)\\$)", x, perl = TRUE), gsub("$", "\\$", x, fixed = TRUE), x)
  paste('{\\textbf{',x,'}}', sep ='')
}
...
if (!file.exists("impact_policy_after_tax_returns_weighted_all.rda") || 
    !file.exists("price_pc_change_of_both_tax_changes_fully_capitalized.rda")){
  impact_policy_after_tax_returns_weighted_all <- 99999
  cgt_reducer_due_to_policy <- 0
} else {
  load("impact_policy_after_tax_returns_weighted_all.rda")
  load("price_pc_change_of_both_tax_changes_fully_capitalized.rda")
  cgt_reducer_due_to_policy <- 
    (-1 * impact_policy_after_tax_returns_weighted_all + price_pc_change_of_both_tax_changes_fully_capitalized) / 2
}
@

<<omit_repeated>>=
omit_repeated <- function(x) {
  if_else(x != lag(x) | is.na(lag(x)), as.character(x), NA_character_)
}
@

<<cgt_reducer_due_to_policy>>=
cgt_reducer_due_to_policy <- cgt_reducer_due_to_policy
@

<<load_sample_file>>=
sample_file <-
  switch (latest.sample.fy, 
          "2015-16" = copy(sample_file_1516),
          "2013-14" = copy(sample_file_1314),
          stop("Unanticipated fy.year."))
@

<<create-age-imp>>=
set.seed(48031)
sample_file <-
  merge(sample_file, age_range_decoder, by = "age_range") %>%
...
target_sample_file <-
  sample_file %>%
  project_to(TARGET.FY, 
             fy.year.of.sample.file = latest.sample.fy)
@
...
hilda_hh <- 
  if (hilda_used) {
    switch (wave, 
            "n" = hildaData::Combined_n150c,
            "q" = hildaData2017::Combined_q170c) %>%
      # copy
      as.data.table %>%
      setnames(paste0(wave, "hhrhid"),
               "id_hh") %>%
      unique(by = "id_hh") %>%
      selector(cols = c("id_hh", "xwaveid",
                        paste0(wave, c(if (wave == "n") "hweqini", "hhwth"))),
               shallow = TRUE) %>%
      .[]
  } else {
    data.table()
  ...
  if (hilda_used) {
    hildaData::Combined_n150c %>%
      ...
      .[, .(nhhwth = sum(nhhwth, na.rm = TRUE)), keyby = c(grep("(hhrhid|hhwth)$", 
                                                                ...
                                                                value = TRUE, 
                                                                invert = TRUE))] %T>%
      fwrite("tsv/hilda_w14_hh.tsv", sep = "\t", na = "NA") %>%
      .[]
  } else {
    fread("tsv/hilda_w14_hh.tsv", sep = "\t", na.strings = "NA")
  }

@

<<Import-HILDA-all-waves-step-1, eval=reload_all_data, cache=FALSE>>=
# read_hilda_file <- 
#   function(filename, max_wave = 14){
#     library(magrittr)
#     library(dplyr)
#     library(data.table)
#     temp <- mutate(data.table(foreign::read.dta(filename)),
#                    orig.filename = filename)
#     nms <- names(temp)
#     # detect the prefix if it is the wave id.
#     nms.prefixes <- unique(gsub("^(.).*$", "\\1", nms))
#     yearid <- nms.prefixes[nms.prefixes %in% letters[1:max_wave]]
#     # we're interested in stripping the names that specify the wave id (a-<letter-of-max-wave-id>>)
#     
#     setnames(temp, 
#              old = nms, 
#              new = gsub(paste0("^[a-", letters[max_wave], "]"), "", nms))
#     
#     wave.decoder <- 
#       data.table(
#         wave = letters[1:max_wave],
#         Year = 2000 + 1:max_wave
#       ) %>%
#       setkey(wave)
#     
...
#   hilda_list <-
#     parLapply(cl = cluster, X = enumerate.person.files, fun = read_hilda_file) 
#   stopCluster(cluster)
# } else {
#   hilda_list <-
#     mclapply(X = enumerate.person.files, FUN = read_hilda_file, mc.cores = cores)
# }
# 
...
<<Import-SIH>>=
if (file.exists(sih13bh.dta <- "~/Data/ABS/IncomeHouse2013-14_Stata/Stata files/sih13bh.dta")) {
  provide.dir("ABS")
  sih201314 <- 
    read_dta(sih13bh.dta) %>%
    as.data.table %>% 
    selector(cols = c("NUMRPRCF", "SIHHHWT")) %>%
    .[, SIHHHWT := round(SIHHHWT, 2)] %>%
    .[]
  
  fwrite(sih201314, "ABS/sih201314.tsv", sep = "\t")
} else {
  sih201314 <- fread("ABS/sih201314.tsv", sep = "\t")
}
@
...
if (!file.exists("ATO/individuals_table1_201314.tsv")) {
  ...
  setnames(old = names(.)[grepl("([12][0-9]{3}).([0-9]{2}|2000)", names(.))],
    ...
    setDT %>%
    fwrite("ATO/individuals_table1_201314.tsv",
           sep = "\t",
           na = "NA")
}

individuals_table1_201314 <- fread("ATO/individuals_table1_201314.tsv", 
                                   sep = "\t",
                                   na.strings = "NA")
stopifnot(grepl("Selected.*item", names(individuals_table1_201314)[1L]))
setnames(individuals_table1_201314, 1L, "Selected_items")

@

<<individuals_table1_201516>>=
if (!file.exists("./ATO/Individuals_table1_201516.xlsx")){
  GET("https://data.gov.au/dataset/d170213c-4391-4d10-ac24-b0c11768da3f/resource/c4ac2c65-7e0c-49bb-adc2-356356a03ab1/download/taxstats2016individual01byyear.xlsx", 
      write_disk("./ATO/Individuals_table1_201516.xlsx"))
}

if (!file.exists("ATO/individuals_table1_201516.tsv")) {
  read_excel("ATO/Individuals_table1_201516.xlsx", sheet = 2, skip = 2, na = "na") %>%
  setnames(old = 2, new = "unit") %>%
  setnames(old = names(.)[grepl("([12][0-9]{3}).([0-9]{2}|2000)", names(.))],
           # endash -> hyphen
           new = gsub("1999-2000", "1999-00", fixed = TRUE,
                      x = gsub("([12][0-9]{3}).([0-9]{2}|2000)", 
                               ...
                               names(.)[grepl("([12][0-9]{3}).([0-9]{2}|2000)", names(.))]))) %>%
    setDT %>% 
    fwrite("ATO/individuals_table1_201516.tsv",
           sep = "\t",
           na = "NA")
}

individuals_table1_201516 <- fread("ATO/individuals_table1_201516.tsv", 
                                   sep = "\t",
                                   na.strings = c("NA",
                                                  # f randomly occurs in 2008-09 column
                                                  '"f"',
                                                  "f"))
stopifnot(!any(vapply(individuals_table1_201516, is.character, FALSE))[-c(1:2)])
stopifnot(grepl("Selected.*item", names(individuals_table1_201516)[1L]))
setnames(individuals_table1_201516, 1L, "Selected_items")
@

<<individuals_table1>>=
individuals_table1 <- 
  switch (latest.sample.fy, 
          "2015-16" = individuals_table1_201516,
          "2013-14" = individuals_table1_201314)
@

<<ABS-housing-commitments>>=
# 2019-19-29
get_ABS_Housing_Commitments <- function(Table = 11,
                                        Today = Sys.Date()) {
  stopifnot(is.integer(Table) || as.integer(Table) == Table,
            dir.exists("ABS"))
  daysSince <- (difftime(Today, as.Date("2018-11-01"), units = "days"))
  if (as.numeric(daysSince) > 92) {
    ## CESSATION OF HOUSING FINANCE, AUSTRALIA (5609.0) AND LENDING FINANCE, AUSTRALIA (5671.0)
    # 
    # November 2018 is the final issue of the Housing Finance, Australia
    # (5609.0) and Lending Finance, Australia (5671.0) publications. Both
    # publications will be combined into a single, simpler publication
    # called Lending to Households and Businesses, Australia (5601.0).
    # An information paper Information Paper: Forthcoming Changes to
    # Monthly Finance Statistics (2018) (5601.0.55.001) was released on
    # 11 January 2019, which outlines both the forthcoming changes in more
    # detail and the format of the new publication, as well as providing
    # historical data in the new table structures. In the new publication,
    # some of the data currently published in the two publications will no
    # longer be published, however, some existing outputs will be further
    # disaggregated. New analytical series will also be added with the
    # inclusion of seasonally adjusted and trend estimates for more data
    # items. This new consolidated publication will be released on 12
    # February 2019.
    
    message("ABS Housing finance data (Cat 5609.0) is out of date by more than a quarter.")
  }
  
  if (file.exists(the.tsv <- sprintf("tsv/5609-9-Table%d.tsv", Table))) {
    out <- fread(the.tsv, sep = "\t")
    out[, Date := as.Date(Date)] 
    return(out)
  }
  
  TableC <- as.character(Table)
  URL <- 
    switch(TableC,
           "11" = {
           "http://www.ausstats.abs.gov.au/ausstats/meisubs.nsf/0/D30BA82E02D34A2ACA25838400152F1F/$File/5609011.xls"
           },
           stop("Undefined Table No."))
  destfile.xls <- sprintf("ABS/5609-9-Table%d.xls", Table)
  
  if (!file.exists(destfile.xls)) {
    res <- download.file(URL, 
                         mode = "wb",
                         destfile = destfile.xls)
    if (res) {
      stop("download failed with status code", res, ".")
    }
  }
  switch(TableC, 
         "11" = {
           # 
           .Description_by_SeriesID <- 
             read_excel(destfile.xls, 
                        sheet = "Data1",
                        range = c("B1:Y10"),
                        .name_repair = "minimal") %>%
             tail(1L) %>%
           ...
           # Can't use melt because of duplicate names
           Description_by_SeriesID <-
             data.table(Description = names(.Description_by_SeriesID),
                        SeriesID = transpose(.Description_by_SeriesID)[[1L]]) %>%
             setkey(SeriesID) %>%
             .[]
           
           .Adjustment_by_SeriesID <- 
             read_excel(destfile.xls, 
                        sheet = "Data1",
                        range = c("B3:Y10"), 
                        col_names = TRUE,
                        # Column names aren't unique but we don't care yet
                        .name_repair = "minimal") %>%
             tail(1L) %>%
             as.data.table
           
          Adjustment_by_SeriesID <- 
            data.table(Adjustment = names(.Adjustment_by_SeriesID),
                       SeriesID = transpose(.Adjustment_by_SeriesID)[[1L]]) %>%
            setkey(SeriesID)
           
           # Units are constant and thousands
           stopifnot(read_excel(destfile.xls,
                                sheet = "Data1",
                                range = "A2",
                                col_names = "V1")[[1L]] == "Unit",
                     identical(apply(read_excel(destfile.xls, 
                                                sheet = "Data1",
                                                range = c("B1:Y2")),
                                     1, 
                                     unique),
            ...
            out <- 
              read_excel(destfile.xls,
                         sheet = "Data1",
                         range = cell_rows(c(10, NA)),
                         .name_repair = "minimal") %>%
              as.data.table %>%
              setnames("Series ID", "Date") %>%
              .[, Date := as.Date(Date)] %>%
              # See above assertion that all units are 1000's
              .[, lapply(.SD, "*", 1000), keyby = "Date"] %>%
              .[] %>%
              melt.data.table(id.vars = "Date",
                              variable.factor = FALSE,
                              variable.name = "SeriesID",
                              value.name = "value") %>%
              setkey(SeriesID, Date) %>%
              .[Description_by_SeriesID, on = "SeriesID"] %>%
              .[Adjustment_by_SeriesID, on = "SeriesID"] %T>%
              fwrite(the.tsv, sep = "\t", na = "NA") %>%
              .[]
...
<<ABS_560911>>=
ABS_560911 <- get_ABS_Housing_Commitments(Table = 11L)
@
...
revenue_CGT_discount <- function(.target_sample_file = target_sample_file,
  ...
  .target_sample_file %>%
    ...
    .[, new_Taxable_Income := pmaxC(Taxable_Income - Net_CG_amt + new_Net_CG_amt, 0)] %>%
...
<<revenue_abolish_NG>>=
revenue_abolish_NG <- function(.target_sample_file = target_sample_file,
                               fy.year = TARGET.FY) {
  .target_sample_file %>%
    copy %>%
    .[, new_Tot_inc_amt := Tot_inc_amt - Net_rent_amt + pmaxC(Net_rent_amt, 0)] %>%
    .[, new_Taxable_Income := pmaxC(new_Tot_inc_amt - Tot_ded_amt - NPP_loss_claimed - PP_loss_claimed, 0)] %>%
    .[, prev_tax := income_tax(Taxable_Income, fy.year = fy.year)] %>% 
...
probNG_if_investor <- 
  sample_file %>%
  .[Gross_rent_amt > 0] %>%
  .[, mean(Net_rent_amt < 0) * (1 - mean(Net_CG_amt > 0))]
@
...
revenue_quarantine_NG_year0 <- function(.target_sample_file,
                                        fy.year = TARGET.FY,
                                        new_CGT_discount = new.discount, 
                                        sample_file = NULL,
                                        sample_file_fy_year = latest.sample.fy, 
                                        .old.discount = 0.50) {
  if (!is.null(sample_file)) {
    stop("arg sample_file deprecated. Use .target_sample_file.")
  }
  if (length(sample_file_fy_year %notin% c("2013-14", "2015-16")) != 1L) {
    stop("Internal error (revenue_quarantine_NG_year0): 557\t",
         length(sample_file_fy_year), "\t", 
         length(sample_file_fy_year %notin% c("2013-14", "2015-16")))
  }
  if (sample_file_fy_year %notin% c("2013-14", "2015-16")) {
  ...
  .target_sample_file %>%
    ...
    .[, new_Net_CG_amt := if_else(runif(.N) < probNoDiscount, 
                                  as.double(Net_CG_amt), 
                                  (1 - cgt_reducer_due_to_policy) * (Net_CG_amt / (1 - .old.discount)) * (1 - new_CGT_discount))] %>%
    ...
    .[, Other_Income := Tot_inc_amt - Net_CG_amt + new_Net_CG_amt - quarantined_income - Net_rent_amt] %>%
    .[, new_Tot_inc_amt := quarantined_income + if_else(Other_Income <= 0, 
                                                        # Nothing for rent to deduct against, 
                                                        # but Other_Income is permitted to be -ve
                                                        as.double(Other_Income),
    ...
    .[, new_Taxable_Income := pmax0(new_Tot_inc_amt - Tot_ded_amt - NPP_loss_claimed - PP_loss_claimed)] %>%
    .[, prev_tax := income_tax(Taxable_Income, fy.year = fy.year)] %>%
    .[, post_tax := income_tax(new_Taxable_Income, fy.year = fy.year)] %>%
...
revenue_quarantine_NG <- function(.target_sample_file = target_sample_file,
                                  sample_file = NULL,
                                  fy.year,
                                  new_CGT_discount = new.discount, 
                                  sample_file_fy_year = latest.sample.fy,
                                  ...
                                  Z = 20,
                                  probNG = NULL) {
  stopifnot(is.null(sample_file))  # avoid confusing sample_file_fy_year
  target_sample_file <- copy(.target_sample_file)
  if (length(sample_file_fy_year %notin% c("2013-14", "2015-16")) != 1L) {
    stop("Internal error: 617",
         length(sample_file_fy_year), "\t", 
         length(sample_file_fy_year %notin% c("2013-14", "2015-16")))
  }
  if (sample_file_fy_year %notin% c("2013-14", "2015-16")) {
  ...
  if (!age_based) {
    ...
    q_by_age <- 
      target_sample_file %>%
      .[Gross_rent_amt > 0] %>%
      .[, .(q = mean(Net_rent_amt < 0)), keyby = "age_range"]
    ...
    q_by_integer_age_long <- 
      ...
      setkey(age, n_ahead) %>%
      .[]
#       age n_ahead prob_NG_if_investor
#    1:  15       1              0.8571
#    2:  15       2              0.8571
#    3:  15       3              0.8571
#    4:  15       4              0.8571
...
# 3600:  74      60              0.1447
  ...
  target_sample_file %>%
    ...
    # assign a new discount rate.  Note that this does not 
    # adequately take into account the impact of capital losses
      # capital gains are impacted by the policy itself.  Assume 2% fall in gains. 
    ...
    .[,
      # This is the standard calculation for taxable income
      new_Taxable_Income_yearZ := pmax0(pmax0(Other_Income + new_Net_CG_amt - accumulated_losses_year_Z) +
                                          quarantined_income - Tot_ded_amt - NPP_loss_claimed - PP_loss_claimed)
      ] %>%
    .[,
      new_Taxable_Income_year0 := pmax0(pmax0(Other_Income + new_Net_CG_amt) +
                                          quarantined_income - Tot_ded_amt - NPP_loss_claimed - PP_loss_claimed)
      ] %>%
    
    .[, prev_tax := income_tax(Taxable_Income, fy.year = fy.year)] %>% 
    .[, post_tax_yearZ := income_tax(new_Taxable_Income_yearZ, fy.year = fy.year)] %>%
...
<<revenue_quarantine_NG_2>>=
# new.discount <- 0.25  # proposed CGT
# TARGET.FY <- "2015-16"
# use.sample.fy <- "2013-14"
# Z is ~ number of iterations of the model for age_based = FALSE
# Try to account for differences in CG by age by guessing the CG losses
# that will be write-offable when the accumulated losses conclude
revenue_quarantine_NG_2 <- function(.target_sample_file = target_sample_file,
                                    sample_file = NULL,
                                    fy.year,
                                    new_CGT_discount = new.discount,
                                    sample_file_fy_year = use.sample.fy,
                                    target_fy = TARGET.FY,
                                    age_based = TRUE,
                                    Z = 20) {
  stopifnot(is.null(sample_file))
  ...
  target_sample_file %>%
    ...
    .[, 
      ...
      accumulated_losses_year_0 := -1 * if_else(Net_rent_amt >= 0, 
                                                0, 
                                                pmin0(pmax0(Other_Income) + Net_rent_amt)) 
      # Not required.
      # ,new_Tot_inc_amt = new_Tot_inc_amt_no_CG + Net_CG_amt
    ] %>%
    .[, rand := runif(.N)] %>%
    ...
    .[, gain_residual := 0* if_else(rand < prob__CG_event & accumulated_losses_year_Z > 0,
                                    pmaxC(expected_tot_cg_given_cg_occurs - accumulated_losses_year_Z, 0), 
                                    0)] %>%
    
      # This is the standard calculation for taxable income
    .[, new_Taxable_Income_yearZ := pmaxC(pmaxC(Other_Income + new_Net_CG_amt + ((1 - new_CGT_discount) / 0.50) * gain_residual - income_reduction, 
...
henry_taxstats <- function(new.discount) {
  ...
  .[, new_Taxable_Income := pmax0(Taxable_Income - Net_CG_amt + new_capital_gains - Net_rent_amt + new_net_rent - Net_fincl_invstmt_lss_amt + new_investment_lss)]
...
# Our proposal
policy <- revenue_quarantine_NG(.target_sample_file = target_sample_file,
                                fy.year = TARGET.FY,
                                new_CGT_discount = 0.25,
                                Z = 50)

policy_alternate_method <- 
  revenue_quarantine_NG_2(.target_sample_file = target_sample_file,
                          new_CGT_discount = 0.25, 
                          fy.year = TARGET.FY, 
                          Z = 50)

policy_year0 <- revenue_quarantine_NG_year0(.target_sample_file = target_sample_file,
                                            fy.year = TARGET.FY)
# The Henry proposal
...
print_cost <- function(the.policy, bn = TRUE){
  stopifnot(!is.null(the.policy$WEIGHT[1]), !is.null(the.policy$tax_increase))
  ...
  formatC(round(the.policy$WEIGHT[1] * sum(the.policy$tax_increase) / 1e0, divisor), 
          width = 4, digits = 1, flag = "#", format = "f")
}

cost_of <- function(the.policy) {
  stopifnot(!is.null(the.policy$WEIGHT[1]),
            !is.null(the.policy$tax_increase))
  the.policy$WEIGHT[1] * sum(the.policy$tax_increase)
}

total_cost_of_proposed_policy <- 
  (cost_of(policy) + cost_of(policy_alternate_method)) / 2

cost_of_NG_only <- 
  total_cost_of_proposed_policy - cost_of(policy_reduce_CGT)
...
write(x = c(
  ...
  print_cost(policy_alternate_method), "Quarantine NG losses and reduce CGT discount (alternate)"
      ),
  file = "Summary_table.txt",
  append = TRUE,
  ncolumns = 2
...
<<revenue_foregone_CG_and_NG>>=
revenue_foregone_CG_and_NG <- 
  cost_of(revenue_abolish_NG(target_sample_file, fy.year = TARGET.FY)) +
  cost_of(revenue_CGT_discount(CGT.discount = 0))
@

<<CostofNGtotal>>=
NG.tx.exp <-
sample_file_1516 %>%
  mutate(new_tax = income_tax(Taxable_Income - pmin0(Net_rent_amt), fy.year = "2015-16"),
         diff = new_tax - income_tax(Taxable_Income, fy.year = "2015-16")) %$%
  sum(diff) %>%
  multiply_by(50 / 1e9) %>%
  round(1)
...
These two measures in combination allow investors to reduce and defer personal income tax, at an annual cost of \Sexpr{texNum(revenue_foregone_CG_and_NG, 2L, dollar = TRUE)} to the public purse. Our proposals to wind back the discount and negative gearing would save the Commonwealth Government about \highlight{\Sexpr{texNum(total_cost_of_proposed_policy, 2L, dollar = TRUE)} a year}. 
...
\highlight{Reducing the capital gains discount to \Sexpr{latex_percent(1 - new.discount)}} would provide a better balance between these competing considerations and would raise about \highlight{\Sexpr{texNum(cost_of(policy_reduce_CGT), 2L, dollar = TRUE)}} a year.
...
Australia should follow international practice, and not deduct losses from passive investments from wage and salary income.  This change would raise \highlight{\Sexpr{texNum(cost_of(policy_year0) - cost_of(policy_reduce_CGT), 2L, dollar = TRUE)}} a year in the short term, falling to \highlight{\Sexpr{texNum(total_cost_of_proposed_policy - cost_of(policy_reduce_CGT), 2L, dollar = TRUE)}} as losses start to be written off against positive investment income. Other proposals, such as restricting negative gearing to new properties or limiting the dollar value of deductions, leave too many problems in place and introduce additional distortions. 

These changes will improve housing affordability -- a little. We estimate housing prices would be up to \Sexpr{latex_percent(-impact_policy_after_tax_returns_weighted_all)} lower than otherwise. Rents won't change much, nor will the rate of new development.  With tight constraints on supply of land suitable for urban housing, most of the impact will be felt via lower land prices. The changes will not cause housing markets to collapse: their effects on prices are small compared to factors such as interest rates and supply of land.
...
\RaggedRight
\begin{enumerate}
\item \textcolor{DarkOrange}{Reduce the capital gains tax discount for individuals and trusts to \Sexpr{latex_percent(1 - new.discount)}} 
\begin{itemize}
...
cgt_by_entity_asset <- 
  switch (latest.sample.fy,
          "2015-16" = {
            cgt_by_entity_asset201516.tsv <- "ATO/CGT-by-entity-and-asset-type-201516.tsv"
            if (file.exists(cgt_by_entity_asset201516.tsv)) {
              fread(cgt_by_entity_asset201516.tsv, sep = "\t")
            } else {
              taxstats2016snapshot05indexofchartdata.xlsx <- "ATO/CGT-by-entity-and-asset-type-201516.xlsx"
              if (!file.exists(taxstats2016snapshot05indexofchartdata.xlsx)) {
                if (download.file("https://data.gov.au/dataset/d170213c-4391-4d10-ac24-b0c11768da3f/resource/8110da3a-bd09-4a71-bac1-48e69ebdae26/download/taxstats2016snapshot05indexofchartdata.xlsx",
                                  destfile = taxstats2016snapshot05indexofchartdata.xlsx,
                                  mode = "wb")) {
                  stop("CGT by entity asset download failed.")
                }
                read_excel(taxstats2016snapshot05indexofchartdata.xlsx,
                           sheet = 16,
                           range = c("A5:D8")) %>%
                  ...
                  mutate(asset_type = factor(gsub(".", " ", gsub(".$b", "", asset_type, fixed = TRUE), fixed = TRUE), 
                         ...
                         value = value_bn * 10^9) %>%
                  as.data.table %T>%
                  fwrite("ATO/CGT-by-entity-and-asset-type-201516.tsv", sep = "\t") %>%
                  .[]
              }
            }
          },
          ...
          stop("Unanticipated TARGET.FY = ", TARGET.FY))
...
<<CGT-by-entity-asset, dev=c('cairo_pdf', 'png'), fig.ext=c('pdf', 'png')>>=
cgt_by_entity_asset %>%
...
Australian entities -- individuals, companies, and superannuation funds -- accrued \Sexpr{texNum(total_CGT, dollar = TRUE)} of income through capital gains in \Sexpr{TARGET.FY}. Individuals accrued \Sexpr{latex_percent(cgt_by_entity %>% filter(Entity == "Individuals") %$% prop)}
of capital gains and gained more from real estate than any other source. (See \Vref{fig:CGT-by-entity-asset}.)

\begin{figure}
\Caption{Capital gains from real estate assets are almost always through individuals}{Taxable net capital gains, \Sexpr{TARGET.FY}}{fig:CGT-by-entity-asset}
...
value_after <- function(n, 
                        initial_principal = 100,
                        tax_r = 0.00, 
                        cgt.on.realization = TRUE,
                        cgt_tax_r = 0.00,
                        cgt_discount = 0.50,
                        div_r = 0.03,
                        cg_r = 0.05){
...
avg_marginal_rate_CG <- 
  sample_file %>%
  .[Net_CG_amt > 0] %>%
  .[, marginal_rate := income_tax(Taxable_Income + 1, fy.year = use.sample.fy) - income_tax(Taxable_Income, fy.year = use.sample.fy)] %$% 
  mean(marginal_rate)
...
avg_marginal_rate_b4_CG <- 
  ...
  .[, .(avg_marginal_rate_weighted = weighted.mean(marginal_rate_b4_CG, Net_CG_amt), 
    ...
    keyby = .(is_in_workforce = between(age_imp, 20, 65))]
...
<<CGT-by-age-taxable-income>>=
sample_file %>%
  .[, .(probCG = mean(Net_CG_amt > 0), 
  ...
  .[, text.label := if_else(`Taxable income percentile` == max(`Taxable income percentile`),
                            as.character(`Age group`), 
                            NA_character_),
    by = "Age group"] %>%
  # choice of 96 cosmetic (puts labels in a reasonable place semantically and aesthetically)
  .[, text.y := mean(probCG[`Taxable income percentile` > 96 & `Taxable income percentile` < 100]),
    by = "Age group"] %>%
  {
    grplot(., aes(x = `Taxable income percentile`, y = probCG, 
                  color = rev(`Age group`), weight = n_taxpayers)) + 
      theme(# gridlines go to far
        panel.grid.major.y = element_blank()) + 
      annotate("segment",
               x = -Inf, 
               xend = 100, 
               color = theGrey,
               y = seq(0, 0.25, by = 0.05), 
               yend = seq(0, 0.25, by = 0.05), 
               size = rel(0.1)) + 
      ...
      annotate("text", 
               x = 101, 
               y = c(0.06, 0.1, 0.15, 0.27), 
               label = c("Below\n30", "30-49", "50-64", "65+"),
               size = 22/(14/5),
               hjust = 0,
               lineheight = 1.0,
               fontface = "bold", 
               colour = rev(c(gpal(3), theGrey))) + 
      scale_y_continuous(label = function(x) percent(x)) +
      scale_x_continuous(limits = c(-5, 112), breaks = c(0, 25, 50, 75, 100), expand = c(0,0)) 
  } 
@

<<CGT-by-age-income>>=
sample_file %>%
  .[, Taxable_Income_less_CG := Taxable_Income - Net_CG_amt] %>% 
  ...
  .[, text.label := if_else(`Taxable income (less CG) percentile` == max(`Taxable income (less CG) percentile`),
                            as.character(`Age group`), 
                            NA_character_),
    by = "Age group"] %>%
  # choice of 96 cosmetic (puts labels in a reasonable place semantically and aesthetically)
  .[, text.y := mean(probCG[`Taxable income (less CG) percentile` > 94 & `Taxable income (less CG) percentile` < 100]),
    by = "Age group"] %>%
  .[, text.y := if_else(`Age group` %pin% "Below", text.y - 0.02, text.y + 0.01)] %>%
         {
           grplot(., aes(x = `Taxable income (less CG) percentile`, y = probCG, 
             ...
             geom_text(aes(label = text.label, y = text.y), 
                       ...
                       na.rm = TRUE) + 
             # Big space to align with bottom chart
...
<<meanPositive>>=
meanPositive <- function(x, na.rm = FALSE) {
  
  # Avoid NaNs
  xo <- as.double(x[x > 0])
  if (l <- length(xo)) {
    sum(xo, na.rm = na.rm) / l
  } else {
    # length zero
    sum(xo, na.rm = na.rm)
  }
...
sample_file %>%
  selector(cols = c("Net_CG_amt", "WEIGHT", "Taxable_Income", "age_imp")) %>%
  .[, Age_group := cut(age_imp, 
                      breaks = c(-Inf, 30, 50, 65, Inf),
                      labels = c("Below\n30", "30-49", "50-64", "65+"), 
                      include.lowest = TRUE, 
                      ...
                      ordered.results = TRUE)] %>%
  .[, Taxable_Income_less_CG := Taxable_Income - Net_CG_amt] %>%
  mutate_ntile("Taxable_Income_less_CG", n = 100) %>% 
  # `Taxable income (less CG) percentile` = ntile(Taxable_Income_less_CG, 100)) %>%
  .[, .(avg_CGT = meanPositive(Net_CG_amt), 
        n_taxpayers = sum(WEIGHT)),
    keyby = c("Taxable_Income_less_CGPercentile", 
              "Age_group")] %>%
  
  # cosmetic
  .[,
    text.label := if_else(Taxable_Income_less_CGPercentile == max(Taxable_Income_less_CGPercentile),
                          as.character(.BY[["Age_group"]]), 
                          NA_character_),
    keyby = "Age_group"] %>%
  .[] %>%
  .[, text.y := mean(avg_CGT[Taxable_Income_less_CGPercentile > 94]), 
    keyby = "Age_group"] %>%
  grplot(aes(x = Taxable_Income_less_CGPercentile, 
             y = avg_CGT, 
             color = Age_group,
             weight = n_taxpayers)) + 
  xlab("Taxable income (less CG) percentile") +
  theme(axis.title.y = element_blank(),
        axis.title.x = element_text(hjust = 0.5,
                                    margin = margin(11, 0, 0, 0, unit = "pt"),
                                    face = "bold")) +  
  # gridlines go to far
  ...
  geom_smooth(size = 1.5, se = FALSE) + 
  # Mark Below\n30 as grey
  scale_color_manual(values = rev(c(gpal(3), theGrey))) + 
  geom_text(aes(label = text.label, y = text.y), 
            hjust = 0, 
            nudge_x = 2,
            fontface = "bold", 
...
@
\begin{figure}
\Caption{Older taxpayers are more likely to have capital gains than younger taxpayers, for all age groups at almost all income levels}{Probability of net capital gains (\Sexpr{latest.sample.fy})}{fig:CGT-by-age-income}
...
ROR_CG <- function(n = 15, 
  ...
  accumulated_capital_gain <- asset_value * nominal.capital.gain.pa
  asset_value <- asset_value * (1 + nominal.capital.gain.pa) + (income - tax)
  
  if (verbose){
    cat("n", "INC", "TX", "AV", "CG", "CGT", "\n", sep = "\t")
    cat("===============================================", "\n")
    cat(1, dollar(income), dollar(tax), dollar(asset_value), "\n", sep = "\t")
  }
  ...
  if (n >= 2){
    for (y in 2:n){
      ...
      accumulated_capital_gain <- accumulated_capital_gain + asset_value * nominal.capital.gain.pa
      # EOY
      asset_value <- asset_value * (1 + nominal.capital.gain.pa) + (income - tax) - cgt
      if (verbose){
        cat(y, dollar(income), dollar(tax), dollar(asset_value), "\n", sep = "\t")
      }
    }
  ...
  if (CGT_on_realization){
    taxable_gain <- 
    capital_gains_tax <- 
      tax_on_savings * (accumulated_capital_gain) * CGT_marginal_rate * (1 - CGT_discount * (n > CGT_avbl_after))
  } else {
    capital_gains_tax <- 0
  }
  
  if (verbose){
    cat(" ", " ", " ", " ", accumulated_capital_gain, capital_gains_tax, "\n", "\n", sep = "\t")
  }
...
data.table(
  ...
  mutate(labels = if_else(if_else(as.character(CGT_regime) %ein% "EMTR (no CGT discount)",
                                  n == 18,
                                  n == 12), 
                          gsub(")", ")\n", 
                               gsub("EMTR", "", as.character(CGT_regime)), 
                               fixed = TRUE), 
                          "")) %>%
  rename(`Holding period for assets (years)` = n) %>%
  mutate(text.x = dplyr::case_when(labels %pin% "no CGT discount" ~ 20,
                                   labels %ein% " (50% CGT discount)\n" ~ 12,
                                   labels %pin% "gains realized at lower rate" ~ 2,
                                   TRUE ~ 2),
         text.y = dplyr::case_when(labels %pin% "no CGT discount" ~ 0.4,
                                   labels %ein% " (50% CGT discount)\n" ~ 0.3,
                                   labels %pin% "gains realized at lower rate" ~ 0.23,
                                   TRUE ~ 0.23)) %>%
  {
    grplot(., aes(x = `Holding period for assets (years)`, 
                  y = EMTR, 
                  colour = CGT_regime)) + 
      xlab("Holding period for assets (years)") +
      geom_line() + 
      geom_text(aes(x = text.x,
                ...
                lineheight = 0.9,
                hjust = 0,
                family = if_knitting("helvet", ""),
                na.rm = TRUE) + 
      scale_y_continuous(label = function(x) paste0(round(100 * x), "%"),
                         breaks = (0:6)/10,
                         limits = c(min(.$EMTR) - 0.05,
                                    max(.$EMTR) + 0.05),
                         expand = c(0, 0)) +
...
\includegraphics[width=\columnwidth]{\Sexpr{atlas}/delaying-realization-causes-reduction-in-effective-tax-rate-1}
\end{figure}
...
<<should_we_get_ABS>>=
# set to FALSE for home stretch -- data will not change
should_we_get_ABS <- FALSE &&
...
res_dwelling_values <- 
  ...
  res_dwelling_values <- 
    fread("res_dwelling_values.tsv") %>% 
    # I added duplicate dates in a failed merge
    unique 
}
@
...
<<return_rental>>=
return_rental <- function(gearing_ratio, return_type = c("real", "nominal", "excess"), after_tax = FALSE, n = 15, CGT.discount = 0.50, inflation = 0.025, risk.free.rate = inflation + 0.02, marginal.tax.rate = 0.47, investment.value = 100, nominal.rent = 0.03, capital.gains = 0.05, interest.rate = 0.05) {
  nominal.return <- capital.gains + nominal.rent
...
<<emtr_rental>>=
emtr_rental <- function(gearing_ratio, return_type = c("real", "nominal", "excess"), n = 15, CGT.discount = 0.50, inflation = 0.025, risk.free.rate = inflation + 0.02, marginal.tax.rate = 0.47, investment.value = 100, nominal.rent = 0.03, capital.gains = 0.05, interest.rate = 0.05, cgt_indexed = FALSE, verbose = FALSE){
  # 
  # # For debugging
  # gearing_ratio = 0.80
  ...
  if (verbose){
    cat("y", "fwd_bal", "rent", "rent_tx", "ann_CG", "nom_gain", "\n", sep = "\t\t")
    cat("============================================================================================================", "\n")
  }
  ...
  for (y in 2:n){
    ...
    if (verbose){
      cat(y, 
          dollar(carry_forward_balance), 
          dollar(rent), 
          dollar(tax_on_rent_minus_interest), 
          dollar(annual_capital_gains), 
          dollar(nominal_gain_since_inception), "\n", sep = "\t\t")
      Sys.sleep(1/4)
    }
  }  
  
  inflation_index <- (1 + inflation) ^ n
  risk_free_return_index <- (1 + risk.free.rate) ^ n
  
  nominal_gain_since_inception <- nominal_gain_since_inception
  
  if (!cgt_indexed){
    CGT_if_gain_realized <- nominal_gain_since_inception * (1 - CGT.discount) * marginal.tax.rate 
  } else {
    real_gain <- investment.value  + nominal_gain_since_inception - investment.value * inflation_index
    CGT_if_gain_realized <- real_gain * marginal.tax.rate
  }
  
  if (verbose){
    cat("\n", "CGT:", "\t", dollar(CGT_if_gain_realized), "\n\n")
    Sys.sleep(1)
  }
  
  nominal_investment_value_before_tax <- 
    investment.value * (1 + nominal.rent + capital.gains) ^ n
  ...
  if (verbose){
    cat("Investment values:\n", 
        "Nominal-post:" , "\t", dollar(nominal_investment_value_after_tax), "\n",
        "Nominal-ante:", "\t", dollar(nominal_investment_value_before_tax),"\n",
        "Real:", "\t\t", dollar(real_investment_after_tax), "\n\n")
  }
  
  
  annualized_real_investment_before_tax <- (real_investment_before_tax / investment.value) ^ (1 / n) - 1
  annualized_real_investment_after_tax  <- (real_investment_after_tax  / investment.value) ^ (1 / n) - 1
  
  annualized_nominal_investment_before_tax <- (nominal_investment_value_before_tax / investment.value) ^ (1 / n) - 1
  annualized_nominal_investment_after_tax  <- (nominal_investment_value_after_tax  / investment.value) ^ (1 / n) - 1
  
  annualized_excess_investment_before_tax <- (excess_returns_before_tax / investment.value) ^ (1 / n) - 1
  annualized_excess_investment_after_tax  <- (excess_returns_after_tax  / investment.value) ^ (1 / n) - 1
...
test_emtr <- function() {
  ...
  expect_equal(emtr_rental(GR, return_type = "nominal", inflation = i1), 
               emtr_rental(GR, return_type = "nominal", inflation = i2))
  ...
  expect_lt(emtr_rental(GR, return_type = "real", nominal.rent = nr, inflation = i1), 
            emtr_rental(GR, return_type = "real", nominal.rent = nr, inflation = i2))
...
<<EMTR-nominal-real-excess-historic-vs-lower>>=
data.table::CJ(
  return_type = c("nominal", "real", "excess"), 
  scenario = c("historic", "lower")
) %>% 
  mutate(marginal_tax_rate = 0.47, 
         marginal_tax_rate_label = if_else(return_type == "nominal" & scenario == "historic", 
                                          "Nominal tax rate", 
                                          NA_character_)) %>%
  rowwise %>%
  mutate(
    emtr = emtr_rental(return_type = return_type,  
                       marginal.tax.rate = marginal_tax_rate, 
                       capital.gains = if (scenario == "historic") 0.073 else 0.050,
                       risk.free.rate = if (scenario == "historic") 0.056 else 0.045,
                       inflation = if (scenario == "historic") 0.028 else 0.025,  
                       CGT.discount = 0.5, 
                       n = 15, gearing = 0, 
                       nominal.rent = 0.03)
  ) %>%
  mutate(`Return type` = factor(return_type, 
                                levels = c("nominal", "real", "excess"), 
                                label = c("Nominal", "Real", "Excess"))) %>%
  grplot(aes(x = `Return type`, fill = scenario, y = emtr)) + 
  geom_bar(stat = "identity", position = "dodge") + 
  scale_y_continuous(label = percent, expand = c(0,0)) + 
  geom_hline(aes(yintercept = marginal_tax_rate), linetype = "dashed", color = theGrey) + 
  geom_text(aes(label = marginal_tax_rate_label, y = marginal_tax_rate), 
            nudge_y = 0.0125, # position = position_dodge(width = 0.9), 
            nudge_x = -0.5,
            size = 22/(14/5),
            color = theGrey,
            vjust = -0.1, 
            ...
            na.rm = TRUE)
@
<<emtr_nominal_historic_lower_assumptions>>=
# Rerun
emtr_nominal_historic_lower_assumptions <- function(){
  percent1 <- function(x) {
    paste0(formatC(round(100 * x, 1), 
                   format = "f", flag = "#", digits = 1, width = 3), "%")
  }
  
  percent1_01 <- function(x) {
    if (is.numeric(x) && between(x, 0, 1)) {
      percent1(x)
    } else {
      x
    }
  ...
  data.table::CJ(
    ...
    .[, marginal_tax_rate := 0.47] %>%
    .[, capital.gains := if_else(scenario == "historic", 0.073, 0.050)] %>%
    .[, risk.free.rate := if_else(scenario == "historic", 0.056, 0.045)] %>%
    .[, inflation := if_else(scenario == "historic", 0.028, 0.025)] %>%
    .[, ConstantXXXmarginal.tax.rate := marginal_tax_rate] %>%
    .[, ConstantXXXCGT.discount := 0.5] %>%
    .[, ConstantXXXn := 15] %>%
    .[, ConstantXXXnominal.rent := 0.03] %>%
    .[, lapply(.SD, percent1_01)]
}
@
\includegraphics[width=\columnwidth]{\Sexpr{atlas}/EMTR-nominal-real-excess-historic-vs-lower-1}
\end{figure}
\begin{table*}
\centering
\caption{Assumptions for \Vref{fig:EMTR-savings}}
\small
<<assumptions-table, results='asis'>>=
emtr_nominal_historic_lower_assumptions() %>% 
  drop_col("marginal_tax_rate") %>%  # duplicated
  setnames(names(.), gsub("[._]", " ", names(.))) %>%
  .[, lapply(.SD, omit_repeated)] %>%
  .[, lapply(.SD, function(x) gsub("(?<!(\\\\))%", "\\\\%", x, perl = TRUE))] %>%
  print_2heading_xtable(separator = "XXX",
                        sanitize.text.function = identity,
                        xtable.align = "lllrrrrrrr",
                        floating = FALSE) 
@
\end{table*}
...
prop_sw_top_decile <- 
  sample_file %>%
  .[, Sw_amt_dbl := as.double(Sw_amt)] %>%  # to avoid integer overflow / gforce warning
  .[, .(tot_sw = sum(Sw_amt_dbl)),
    keyby = .(top_sw_decile = ntile(Sw_amt, 10) == 10)] %>%
  .[, prop_sw := tot_sw / sum(tot_sw)] %>%
  .[(top_sw_decile)] %$%
  prop_sw

prop_invst_inc_top_decile <- 
  sample_file %>%
  .[ntile(Tot_inc_amt, 10) == 10] %>%
  .[, investment_income := Tot_inc_amt - Sw_amt - Alow_ben_amt - ETP_txbl_amt - Aust_govt_pnsn_allw_amt - Other_foreign_inc_amt] %$%
  mean(investment_income / Tot_inc_amt)

prop_invst_inc_from_top_decile <- 
  sample_file %>%
  .[, investment_income := Tot_inc_amt - Sw_amt - Alow_ben_amt - ETP_txbl_amt - Aust_govt_pnsn_allw_amt - Other_foreign_inc_amt] %>%
  .[, .(tot_invest = sum(as.numeric(investment_income))),
    by = .(decile = ntile(Tot_inc_amt, 10))] %>%
  .[, prop_invest := tot_invest / sum(tot_invest)] %>%
  .[decile == 10] %$%
  prop_invest
...
prop_inc_CG_top_decile_excl_CG <- 
  sample_file %>%
  .[, .(tot_CG = sum(Tot_CY_CG_amt)),
  by = .(decile = ntile(Tot_inc_amt - Net_CG_amt, 10))] %>%
  .[, prop_CG := tot_CG / sum(tot_CG)] %>%
  .[decile == 10] %$%
  prop_CG
...
\begin{figure}
\Caption{Most capital gains are earned by those in the highest income decile}{Proportion of net capital gains by income decile (\Sexpr{latest.sample.fy})}{fig:CG-by-decile}
<<CG-by-decile>>=
sample_file %>%
...
<<sih_prop_assets_in_OO_super>>=
sih_prop_assets_in_OO_and_super <- 
  # From website at 2016-04-19
  # Cat 6523.0 Household Income and Wealth, Australia 2013-14
  # 16 December 2015
  # Table 8.4
  0.418 + 0.167
...
Second, reducing the discount might not lead to much greater investment in owner-occupied housing and superannuation. Households already hold \Sexpr{latex_percent(1 - sih_prop_assets_in_OO_and_super)} of their assets outside these areas, even though they have substantial tax advantages. This is presumably because people want savings in forms that are available for use before retirement and without having to sell their home.  
...
pDecile <- 
  ...
  arrange(type, Taxable_Income_decile) %T>%
  fwrite("CGT-NG-atlas/Distribution-taxes-income-vs-taxable-income-decile-1.csv") %>%
...
pDecile_no_CG <- 
  ...
  mutate(type = factor(type, levels = unique(.$type), ordered = TRUE)) %>%
  arrange(type, Taxable_Income_decile) %T>%
  fwrite("CGT-NG-atlas/Distribution-taxes-income-vs-taxable-income-decile-2.csv") %>%
  grplot(aes(x = type, y = value, fill = factor(Taxable_Income_decile, levels = 10:1))) + 
...
<<dan-box>>=
dan_boxer <- function(n, 
                      interest_only = TRUE, 
                      loan_period = 20,
                      sw = 250e3, 
                      borrow = 750e3,
                      initial_property_value = 750e3, 
                      loan.interest = 0.06, 
                      rent_return = 0.025, 
                      capital_return = 0.05, 
                      cgt_discount = 0.50, 
  ...
  # intial
  if (verbose){
    cat("year", "Property value", "Net rent", "Tax red.", "Cap. gain", "\n", 
        "------------------------------------------------------------------", "\n", sep = "\t")
  }
  ...
  if (interest_only){
    loan_repayment <- loan.interest * borrow  # interest-only
    interest_payment <- loan_repayment
  } else {
    loan_outstanding <- borrow
    loan_repayment <- loan.interest * borrow / (1 - (1 + loan.interest) ^ (-loan_period))
    interest_payment <- loan_outstanding * loan.interest
  }
  
  rent <- rent_return * property_value
  net_rent <- rent - interest_payment
  tax <- income_tax(sw + net_rent, "2015-16")
  tax_reduction <- income_tax(sw, "2015-16") - tax
  property_value <- property_value * (1 + capital_return)
  loan_outstanding <- borrow - loan_repayment
  
  total_tax_paid <- tax
  total_tax_redu <- tax_reduction
  nominal_pre_tax_profit <- net_rent
  y <- 1
  
  
  if (verbose){
    .d <- function(x) grattan_dollar(x)
    cat(y, .d(property_value), .d(net_rent), .d(tax_reduction), "\n", sep = "\t")
    Sys.sleep(1)
  }
  if (n >= 2){
    for (y in 2:n){
      ...
      rent <- rent_return * property_value  # soy
      net_rent <- rent - interest_payment
      tax <- income_tax(sw + net_rent, "2015-16")
      tax_reduction <- income_tax(sw, "2015-16") - tax
      total_tax_paid <- total_tax_paid + tax
      total_tax_redu <- total_tax_redu + tax_reduction
      property_value <- property_value * (1 + capital_return)  # eoy
      loan_outstanding <- loan_outstanding - loan_repayment
      nominal_pre_tax_profit <- nominal_pre_tax_profit + net_rent
      if (verbose){
        cat(y, .d(property_value), .d(net_rent), .d(tax_reduction), "\n", sep = "\t")
        Sys.sleep(1)
      }
  ...
  if (verbose){
    vcapital_gain <- property_value - initial_property_value
    vtax_redu <- total_tax_redu + income_tax(sw, 
                                           "2015-16") - income_tax(sw + vcapital_gain * (1 - cgt_discount), 
                                                                   "2015-16")
    cat(y, .d(property_value), .d(net_rent), .d(vtax_redu), "", .d(property_value - initial_property_value), 
        "\n",
        sep = "\t")
    Sys.sleep(2)
  }
  ...
  list(
    ...
    capital_gain = capital_gain <- property_value - initial_property_value,
    tax_final = tax_final <- income_tax(sw + net_rent + capital_gain * (1 - cgt_discount), "2015-16"),
    tax_redu_no_CGT = total_tax_redu + income_tax(sw, "2015-16") - income_tax(sw + net_rent, "2015-16"),
    tax_redu = total_tax_redu + income_tax(sw, 
                                           "2015-16") - income_tax(sw + capital_gain * (1 - cgt_discount), 
                                                                   "2015-16"),
    CGT = tax_final - tax,
    total_tax_paid = total_tax_paid + tax_final,
    # profit already incorporates net rent
    nominal_pre_tax_profit = nominal_pre_tax_profit + capital_gain
  )
}
...
# defaults
dan.fml <- formals(dan_boxer)
@

Suppose Dan, a lawyer earning \Sexpr{texNum(dan.fml$sw, dollar = TRUE)} a year, 
borrows \Sexpr{texNum(dan.fml$borrow, dollar = TRUE)} %
\Sexpr{if (dan.fml$interest_only) "(interest only)" else paste("over", dan.fml$loan_period, "years")}
to purchase a \Sexpr{texNum(dan.fml$initial_property_value, dollar = TRUE)} investment property. Interest on the loan is \Sexpr{latex_percent(dan.fml$loan.interest)} a
year and the property generates a rental return of \Sexpr{latex_percent(dan.fml$rent_return)} each
year. Most of the property's return is via capital appreciation of \Sexpr{latex_percent(dan.fml$capital_return)} each year.

In the first year, Dan makes a loss of \Sexpr{texNum(-1 * dan_1yr$net_rent, dollar = TRUE)} 
on the property and
reduces the tax he pays on his \Sexpr{texNum(dan.fml$sw, dollar = TRUE)} salary by 
\Sexpr{texNum(dan_1yr$tax_reduction, dollar = TRUE)}. His rental
...
\Sexpr{texNum(dan_5yr$tax_redu_no_CGT, dollar = TRUE)}. If he
sells the property after five years he will realise a capital gain of
\Sexpr{texNum(dan_5yr$capital_gain, dollar = TRUE)} and pay tax on 
the gain of \Sexpr{texNum(dan_5yr$CGT, dollar = TRUE, pre.phrase = c("just under", "just over"))}.

Because of the asymmetry of tax treatment of gains and losses, Dan pays
\Sexpr{texNum(dan_5yr$tax_redu, dollar = TRUE)} less tax in total over five years than if he had not purchased
the house. So despite his profit of more than \Sexpr{texNum(dan_5yr$nominal_pre_tax_profit, dollar = TRUE)} on the
...
sample_file %>%
  ...
  setnames("age", "x") %>%
  setnames("prop", "y") %>%
  setnames("gearing", "fill") %>%
  
  .[, fill := factor(if_else(fill == "prop_PG",
                             "Earning\nrental income\nbut not\nnegatively\ngeared",
                             "Negative\ngearing"),
                     levels = c("Earning\nrental income\nbut not\nnegatively\ngeared",
                                "Negative\ngearing"),
  ...
  #     scale_y_continuous(label = function(x) paste0(round(100 * x), "%"),
  #                        expand = c(0, 0)) + 
  #     scale_x_continuous("Age", breaks = c(30, 50, 70)) + 
  #     annotate("text", 
  #              x = 76, 
  ...
  #              hjust = 0,
  #              colour = gpal(2)) + 
  #     geom_blank(aes(x = 86, y = 0))
  # }
@
\includegraphics[width=1.225\columnwidth]{\Sexpr{atlas}/NG-PG-by-age-1}
...
tot_intrst_unincorp_biz <- 
  individuals_table1 %>%
  .[grepl("Interest expenses within Australia", Selected_items)] %>%
  .[grepl("$", unit, fixed = TRUE)] %>%
  .subset2(latest.sample.fy)
...
\notes{Assumes 
\Sexpr{latex_percent(emtr_defaults$nominal.rent)} nominal rent income, 
...
individuals_table1 %>%
  # Need to use regex because of text weirdness
  .[grepl("Net.*rent", Selected_items)] %>%
  melt.data.table(id.vars = c("Selected_items", "unit"),
                  variable.name = "fy_year",
                  variable.factor = FALSE,
                  value.name = "value") %>%
  dcast.data.table(... ~ unit) %>%
  .[or(!is.na(`$`), !is.na(no.))] %>%
  .[grepl("Net.rent.*loss$", Selected_items)] %>%
  .[!grepl("property", Selected_items)] %>%
  .[, avg_loss := -`$` / no.] %>% 
  .[, avg_loss_real := cpi_inflator(avg_loss,
                                    from_fy = fy_year,
                                    to_fy = TARGET.FY,
                                    adjustment = "none")] %>%
  
  .[, year := fy2yr(fy_year) + 0.5] %T>%
  {
    grattanCharts:::chart_data(.)
    min.fy <<- min(.$fy_year)
    max.fy <<- max(.$fy_year)
...
\caption*{Average net rental loss, (\Sexpr{TARGET.FY} dollars)}\label{fig:number-NG-and-net-losses-time-series}
...
individuals_table1 %>%
  ...
  dcast.data.table(... ~ unit) %>%
  .[or(!is.na(`$`), !is.na(no.))] %>%
  .[grepl("Net.rent.*(loss|profit)$", Selected_items)] %>%
  .[!grepl("property", Selected_items)] %>%
  .[, avg_loss := -`$` / no.] %>% 
  .[, avg_loss_real := cpi_inflator(avg_loss,
                                    from_fy = fy_year,
  ...
  .[, year := fy2yr(fy_year) - 0.5] %>%
  grplot(aes(x = year, y = no.)) +
  geom_bar(aes(fill = Selected_items),
           stat = "identity", position = "stack") + 
  scale_y_continuous(label = comma, expand = c(0, 0)) + 
  scale_x_continuous(expand = c(0.01, 0.01),
                     breaks = c(1995, 2005, 2015)) + 
  theme(axis.title.x = element_blank()) + 
  geom_text(data = data.table(x = 2016.35, 
                              y = c(0.5e6, 1.5e6),
                              label = c("Negative\ngearing", "Positive\ngearing"), 
                              colour = gpal(2, reverse = TRUE)),
            mapping = aes(x, y, label = label, colour = colour),
            hjust = 0,
            lineheight = 0.75, 
            fontface = "bold") + 
  geom_blank(aes(x = 2019.75, 
                 y = 0))
@
...
prop_rental_deductions_gross_rent <-
  individuals_table1 %>%
  .[grepl("(Rent.*interest)|(Gross rent)", Selected_items)] %>%
  .[grepl("$", unit, fixed = TRUE)] %>%
  drop_col("unit") %>%
  melt.data.table(id.vars = c("Selected_items"),
                  variable.factor = FALSE,
                  variable.name = "fy_year",
                  value.name = "value") %>%
  .[, all_not_na := if (anyNA(value) && all(is.na(value))) FALSE else TRUE, keyby = "fy_year"] %>%
  .[(all_not_na)] %>%
  .[, .(prop = last(value) / first(value)), keyby = "fy_year"] %T>%
  fwrite("tsv/prop_rental_deductions_gross_rent", sep = "\t") %>%
  .[]

# proportion of rental deductions against gross rent (as a percentage
# to three decimal places).
prdgr <- function(fyyear) { 
  o <- prop_rental_deductions_gross_rent[.(fyyear), on = "fy_year"][["prop"]]
  round(o, 3) * 100
}
@
 
 And the average level of gearing for property investors is increasing.
 Even though interest rates are falling, interest deductions as a proportion of rents increased from \Sexpr{prdgr("1997-98")}~per cent of gross rental payments in \mbox{1997-98} to \Sexpr{prdgr("2007-08")} in 2007-08. With falling interest rates, interest deductions as a proportion of gross rents had fallen to \Sexpr{prdgr(latest.sample.fy)}~per cent in \Sexpr{latest.sample.fy}.
...
stopifnot(length(n_NG) == 1L,
          length(val_NG) == 1L)
@

As a result, Australian landlords have moved from being collectively profitable, to accruing billions in net rental losses each year. In \Sexpr{latest.sample.fy}, \Sexpr{texNum(n_NG)} landlords reported collective losses of \Sexpr{texNum(-val_NG, dollar = TRUE)}. Consequently, total net rents have been consistently negative since the introduction of the CGT discount. (\Vref{fig:Net-rent-time-series}).  Losses are reducing only because interest rates have fallen rapidly.

\begin{figure}
\Caption{Since the introduction of the capital gain tax discount, rental losses have been large, overwhelming profits}{Total real net rent (profit \(-\) loss), individual taxpayers, (\Sexpr{TARGET.FY} dollars)}{fig:Net-rent-time-series}
<<Net-rent-time-series, dev='cairo_pdf'>>=
individuals_table1_201314 %>%
  #filter(grepl("Net.rent...((loss)|(profit))$", `Selected items`), unit == "$") %>%
  filter(grepl("Net rent2", Selected_items), unit == "$") %>%
  gather(fy_year, nominal_value, -Selected_items, -unit) %>%
  filter(!is.na(nominal_value)) %>%
  mutate(real_value = cpi_inflator(nominal_value, from_fy = fy_year, to_fy = TARGET.FY)) %>%
  mutate(year = fy2yr(fy_year) + 0.5) %>%
  arrange(fy_year, Selected_items) %>%
...
<<prop_val_loans_that_are_investors>>=
prop_val_loans_that_are_investors <-
  ABS_560911 %>%
  .[!startsWith(Description, "Total")] %>%
  .[!grepl("Refinanc", Description)] %>%
  .[Date == max(Date)] %>%
  .[, c("InvestorOrOwner", "Description") := tstrsplit(Description, split = " - ", fixed = TRUE)] %>%
  .[, InvestorOrOwner := if_else(startsWith(InvestorOrOwner, "Investment"),
                                 "Investor",
                                 "Owner-occupier")] %>%
  .[Adjustment %ein% "Trend"] %>%
  .[, .(value = sum(value)), keyby = "InvestorOrOwner"] %>%
  .[, prop := value / sum(value)] %>%
  .[.("Investor")] %>%
  .[["prop"]] %>%
  latex_percent
@
<<prop_val_loans_that_are_investors_peak>>=
prop_val_loans_that_are_investors_peak <-
  ABS_560911 %>%
  .[!startsWith(Description, "Total")] %>%
  .[!grepl("Refinanc", Description)] %>%
  .[year(Date) >= year(max(Date)) - 20L] %>%
  .[, c("InvestorOrOwner", "Description") := tstrsplit(Description, split = " - ", fixed = TRUE)] %>%
  .[, InvestorOrOwner := if_else(startsWith(InvestorOrOwner, "Investment"),
                                 "Investor",
                                 "Owner-occupier")] %>%
  .[Adjustment %ein% "Trend"] %>%
  .[, .(value = sum(value)), keyby = c("Date", "InvestorOrOwner")] %>%
  .[, prop := value / sum(value), keyby = "Date"] %>%
  .[InvestorOrOwner %ein% "Investor"] %>%
  .[which.max(prop)]
@
<<prop_val_loans_that_are_investors_2decades_ago>>=
prop_val_loans_that_are_investors_2decades_ago <-
  ABS_560911 %>%
  .[!startsWith(Description, "Total")] %>%
  .[!grepl("Refinanc", Description)] %>%
...
The favourable tax treatment reduces home ownership because it increases the after-tax returns to geared property investors but not homeowners. The increase in geared investing has crowded out purchases by owner-occupiers. Investors now account for \Sexpr{prop_val_loans_that_are_investors} of new loans for housing, compared with \Sexpr{prop_val_loans_that_are_investors_2decades_ago} two decades ago.  This is one of the reasons (but by no means the only reason) that rates of home ownership are falling among younger age groups. 

(The proportion of loans going to investors peaked in \Sexpr{prop_val_loans_that_are_investors_peak[, year(Date)]} at \Sexpr{prop_val_loans_that_are_investors_peak[, latex_percent(prop)]}.)

\begin{figure}
<<valueNewLoans-vs-Date-by-Investor, fig.height=1.5*fig_height, out.height=paste0(1.5*out_height, "in"), fig.width=1.1*fig_width, out.width=paste0(1.1*out_width, "in")>>=
ABS_560911 %>%
  .[!startsWith(Description, "Total")] %>%
  .[!grepl("Refinanc", Description)] %>%
  .[year(Date) > 2007L] %>%
  ...
  .[, InvestorOrOwner := if_else(startsWith(InvestorOrOwner, "Investment"),
                                 "Investor",
                                 "Owner-occupier")] %>%
  .[, InvestorOrOwner := factor(InvestorOrOwner, 
                                levels = c("Owner-occupier",
                                           "Investor"),
                                ordered = TRUE)] %>%
  .[Adjustment %ein% "Trend"] %>%
  .[, Description := trim_common_affixes(Description)] %>%
  .[, Description := factor(Description, levels = unique(.$Description), ordered = TRUE)] %T>%
  chart_data %>%
  grplot(aes(x = Date, y = value, fill = Description)) + 
  geom_area(color = "white") + 
  facet_wrap(~InvestorOrOwner) + 
...
\units{Value of housing finance commitments, by purpose}
\includegraphics[width=\columnwidth]{\Sexpr{atlas}/valueNewLoans-vs-Date-by-Investor-1}
\source{ABS Cat 5609.0, Table 11.}
\end{figure}
...
<<prop_le_80k>>=
prop_le_80k <- 
  mean(sample_file$Taxable_Income > 80e3)
@

A lot of the public debate has centred on claims that most taxpayers negatively gearing have taxable incomes below \$80,000. This is not surprising given that only \Sexpr{latex_percent(prop_le_80k)} of taxpayers have higher taxable incomes. 
...
<<median_tx_inc_by_NG>>=
median_tx_inc_by_NG <- 
  sample_file_1314[, .(median = median(Taxable_Income)),
                   keyby = .(NG = Net_rent_amt < 0)]
@
...
sample_file_1314 %>%
  ...
  grplot(aes(Taxable_Income, color = Net_rent_amt >= 0, fill = Net_rent_amt >= 0)) + 
  geom_density(alpha = 0.4, size = 1.5) +
  scale_x_continuous("Taxable Income",  # Label to avoid ticks
  ...
  annotate("segment", 
           ...
           yend = c(density_other$y[with(density_other,
                                         which.max(x[x < median_tx_inc_by_NG$median[1]]))], 
                    density_NG$y[with(density_NG,
                                      which.max(x[x < median_tx_inc_by_NG$median[2]]))]),
           size = 1.2,
           linetype = "dashed", 
           colour = rev(gpal(2))
           ) + 
...
\begin{figure}
\Caption{More taxpayers on higher incomes negatively gear (\Sexpr{latest.sample.fy})}{Distribution of taxable incomes by negative gearing status, \Sexpr{latest.sample.fy}}{fig:tx-inc-distr-by-NG-latest}
% dev omit emf due to alpha
<<tx-inc-distr-by-NG-latest, dev = c('my_pdf', 'png'), fig.ext = c("pdf", "png")>>=
median_tx_inc_by_NG_latest <- 
  sample_file[, .(medianTI = median(Taxable_Income)), keyby = .(NG = Net_rent_amt < 0)]
median_tx_inc_by_NG_latest[, medianTI := as.integer(medianTI)]
...
sample_file %>%
  ...
  scale_y_continuous(expand = c(0,0)) + 
  geom_blank(aes(x = 0, 
                 y = max(density_other_latest$y, density_NG_latest$y) * 1.03)) + 
  coord_cartesian() +
  annotate("segment", 
           x = median_tx_inc_by_NG$median, 
           xend = median_tx_inc_by_NG$median,
           y = -Inf,
           # use max(x) such that x < median
           yend = c(density_other_latest$y[with(density_other_latest, 
                                                which.max(x[x < median_tx_inc_by_NG_latest$median[1]]))], 
                    density_NG_latest$y[with(density_NG_latest, 
                                             which.max(x[x < median_tx_inc_by_NG_latest$median[2]]))]),
           size = 1.2,
           linetype = "dashed", 
           colour = rev(gpal(2))
           ) + 
  annotate("text", 
           x = median_tx_inc_by_NG$median + 1000, 
           y = c(density_other_latest$y[with(density_other_latest, 
                                             which.max(x[x < median_tx_inc_by_NG$median[1]]))], 
                 # 1.02 cosmetic: else label too close to density
                 density_NG_latest$y[with(density_NG_latest,
                                          which.max(x[x < median_tx_inc_by_NG$median[2]]))]) * 1.02, 
           label = c(paste("Not negatively gearing\nMedian:", dollar(median_tx_inc_by_NG_latest$median[1])), 
                     paste("Negatively gearing\nMedian:", dollar(median_tx_inc_by_NG_latest$median[2]))), 
           # size = 21/(14/5),
           hjust = 0,
           lineheight = 0.85,
           fontface = "bold",
  ...
  theme(axis.title.y = element_blank(),
        ...
        axis.line.y = element_blank())
@
\includegraphics[width=\columnwidth]{\Sexpr{atlas}/tx-inc-distr-by-NG-latest-1}
\end{figure}


As shown in \Vref{fig:tx-inc-distr-by-NG}, people with higher incomes are more likely to negatively gear. The median taxable incomes for taxpayers that negatively gear is \Sexpr{texNum(median_tx_inc_by_NG_latest %>% filter(NG) %$% medianTI, dollar = TRUE)} compared to  \Sexpr{texNum(median_tx_inc_by_NG_latest %>% filter(!NG) %$% medianTI, dollar = TRUE)} for those taxpayers that don't negatively gear.
...
benefits_by_taxable_income <- 
  ...
  mutate(Taxable_Income_Decile = ntile(Taxable_Income, 10),
         ...
         benefit_due_to_noneggearg = income_tax(Taxable_Income_NoNegGearg, fy.year = latest.sample.fy) - income_tax(Taxable_Income, fy.year = latest.sample.fy)) %>%
...
@
\begin{figure}
\Caption{Negative gearing mostly benefits those on high incomes, and the difference is especially stark when incomes are measured before subtracting rental interest deductions}{Percentage of each decile's share of the benefit in reduced income tax due to negative gearing (\Sexpr{latest.sample.fy})}{fig:Benefit-NG-before-after-deductions}
...
\source{\gao\ \Sexpr{latest.sample.fy} sample file}
\end{figure}
...
impact_policy_after_tax_returns <- 
  ...
  mutate(
    ...
    `Return Proposed`  = return_rental(gearing_ratio = gearing_ratio, CGT.discount = 0.25, 
                                       return_type = "nominal", after_tax = TRUE)
  ) 

impact_policy_after_tax_returns_weighted_all <- 
  impact_policy_after_tax_returns %>%
  mutate(change_in_return = if_else(`Owner type` == "Owner-occupied", 
                                   0, 
...
save(impact_policy_after_tax_returns_weighted_all, 
     file = "impact_policy_after_tax_returns_weighted_all.rda")
@
...
The proposed changes to negative gearing would reduce tax write-offs for property investment by about \Sexpr{texNum(cost_of_NG_only_year0, sig = 2L, dollar = TRUE)} initially and \Sexpr{texNum(cost_of_NG_only, sig = 2L, dollar = TRUE)} over time. Assuming a discount rate of 
...
The proposed reduction of the capital gains tax discount to \Sexpr{latex_percent(new.discount)} would reduce after tax returns by about \Sexpr{texNum(cost_reduce_CG_discount_25pc, dollar = TRUE)} a year, or \Sexpr{texNum(val_reduce_CG_discount_25pc_perpertuity, dollar = TRUE)} in perpetuity. Assuming \Sexpr{latex_percent(CG_in_real_estate)} of this relates to gains on real estate (in line with \Sexpr{latest.sample.fy} gains realisations) then the expected present value of the lost benefits for would be approximately \Sexpr{texNum(val_reduce_CG_discount_25pc_real_estate_perpertuity, dollar = TRUE)}.
...
save(price_pc_change_of_both_tax_changes_fully_capitalized, 
     file = "price_pc_change_of_both_tax_changes_fully_capitalized.rda")
@
...
<<impact-of-policy-changes-on-after-tax-returns-tbl, results='asis', eval=FALSE>>=
impact_policy_after_tax_returns %>% 
  setDT %>%
  .[, "Return \\ensuremath{\\Delta} \\%" := (`Return BAU` - `Return Proposed`) / `Return BAU`] %>% 
  drop_col("gearing_ratio") %>%
  .[, lapply(.SD, percent1), keyby = "Owner type"] %>%
  
    .[`Owner type` == "Owner-occupied", "Return BAU" := "-"] %>%
    .[`Owner type` == "Owner-occupied", "Return Proposed" := "-"] %>%
    .[`Owner type` == "Owner-occupied", "Return \\ensuremath{\\Delta} \\%" := "-"] %>%
  setnames("Return Proposed", "Return \\mbox{Proposed}") %>%
  .[, lapply(.SD, function(x) gsub("(?<!(\\\\))%", "\\\\%", x, perl = TRUE))] %>%
  {
...
impact_policy_after_tax_returns %>% 
 ...
 .[, "Return \\ensuremath{\\Delta} \\%" := (`Return BAU` - `Return Proposed`) / `Return BAU`] %>% 
  ...
  .[, lapply(.SD, percent1), keyby = "Owner type"] %>%
  mutate(
    `Return BAU` = if_else(`Owner type` == "Owner-occupied", "-", `Return BAU`), 
    `Return Proposed` = if_else(`Owner type` == "Owner-occupied", "-", `Return Proposed`), 
    `Return \\ensuremath{\\Delta} \\%`  = if_else(`Owner type` == "Owner-occupied", "-", `Return \\ensuremath{\\Delta} \\%`)) %>%
...
rental_losses <- 
  sample_files_all[Net_rent_amt < 0, .(Net_rent_amt, WEIGHT, fy.year)] %>%
  .[, Net_rent_amt_Wt := -1 * Net_rent_amt * WEIGHT] %>%
  .[, .(tot_avg_loss = sum(Net_rent_amt_Wt)),
    keyby = "fy.year"] %>%
  .[, real_tot_avg_loss := cpi_inflator(tot_avg_loss, from_fy = .BY[[1L]], to_fy = TARGET.FY), 
    keyby = "fy.year"]

NG_tax_exps <-
  sample_files_all[, .(fy.year, Taxable_Income, Net_rent_amt, WEIGHT)] %>%
  .[, current_tax := income_tax(Taxable_Income, fy.year = .BY[[1L]]), keyby = "fy.year"] %>%
  .[, subseqt_tax := income_tax(if_else(Net_rent_amt < 0, 
                                        Taxable_Income - Net_rent_amt,
                                        Taxable_Income),
                                fy.year = .BY[[1L]]),
    keyby = "fy.year"] %>%
  .[, diff := WEIGHT * (subseqt_tax - current_tax)] %>%
  .[, .(tot.diff = sum(diff)), keyby = "fy.year"] %T>% 
  fwrite("tsv/NG_tax_exps.tsv", sep = "\t") %>%
  .[]

NG_tax_exps.real <- 
  NG_tax_exps %>%
  .[, avg.diff.real := cpi_inflator(tot.diff, from_fy = .BY[[1]], to_fy = TARGET.FY), 
    keyby = "fy.year"] %$%
  mean(avg.diff.real)
...
housing_finance_commitments.metadata <-
  ...
  #
  # 
  read_excel("ABSP_567108_Housing_Finance_Table8.xlsx",
             sheet = "Metadata") %>%
  as.data.table %>%
  selector(cols = c("Series ID", "Long_name", "Series Type")) %>%
  setnames(c("series_id", "Long_name", "Series_Type")) %>%
  setkeyv("series_id") %>%
  .[]

housing_finance_commitments <-
  read_excel("ABS_Cat5671-0_Lending_Finance_Table8_Finance_Commitments_For_housing.xlsx",
             sheet = "Data_no_metadata") %>%
...
value.of.ng.by.yr <- 
  sample_files_all[, .(Taxable_Income, Net_rent_amt, fy.year, WEIGHT)] %>%
  .[, benefit_due_NG := income_tax(pmaxV(Taxable_Income, Taxable_Income - Net_rent_amt), 
                                   fy.year = .BY[[1L]]) - income_tax(Taxable_Income, 
                                                                     fy.year = .BY[[1]]),
    keyby = "fy.year"] %>%
  .[, .(total_benefit = sum(benefit_due_NG * WEIGHT)), keyby = "fy.year"] %>%
  .[, total_benefit_real := cpi_inflator(total_benefit, from_fy = .BY[[1L]], to_fy = TARGET.FY),
    keyby = "fy.year"]
...
invisible(fwrite(rent.indices.real, "ABS/1980s_rent_indices_real.tsv", sep = "\t"))

rent.indices.real  %>%
  filter(!grepl("All", city)) %>%
  mutate(up = factor(if_else(up, "Rents rose", "Rents fell or flattened"))) %>%
  filter(obsTime < as.POSIXct("1989-06-01")) %T>%
  {
    data <- .
    grattanCharts:::chart_data(data)
    } %>%
  {
  # chart_data %>%
  ...
  annotate("rect", 
           ...
           fill = grey(0.8),
           alpha = 1) + 
      geom_blank(aes(x = as.POSIXct("1981-07-01"), 
                     y = 1)) + 
  geom_line() + 
  geom_text(aes(x = obsTime + 60*60*24*60, vjust = vjust, hjust = hjust),
            data = mutate(filter(., obsTime == unique(obsTime)[14]),
                          ...
                          vjust = if_else(grepl("Aust", city), -0.2, 
                                         if_else(grepl("Brisbane", city), 2.3,
                                                if_else(grepl("Perth", city),1.8, 
                                                       if_else(grepl("Sydney", city), -1.4,
                                                              if_else(grepl("Adel", city), -0.7, -1.1))))), 
                          hjust = if_else(grepl("(Adel)|(Per)", city), -0.175, 1)),
            size = 22.5/(14/5), 
            fontface = "bold",
            lineheight = 0.8) +
  ...
  geom_text(aes(x = obsTime + 60*60*24*60, vjust = vjust),
            data = mutate(filter(rent.indices.real, obsTime == unique(obsTime)[14]),
                          ...
                          vjust = if_else(grepl("Aust", city), -0.2, 
                                         if_else(grepl("Brisbane", city), -2.3,
                                                if_else(grepl("Perth", city),-1.8, 
                                                       if_else(grepl("Sydney", city), -1.4, -1.1)))), 
                          hjust = if_else(grepl("(Melbourne)|(Aust)", city), 1, 1)),
            hjust = 1,
            # size = 22.5/(14/5), 
            fontface = "bold",
            lineheight = 0.8) +
...
<<ABS-lending-data>>=
prop_investment_housing_finance_commitments_to_dwelling_construction <- 
  housing_finance_commitments %>%
  filter(grepl("Investment", Long_name), Date == max(Date), Series_Type == "Trend") %>%
  mutate(new_property = grepl("Construction of dwelling", Long_name)) %>%
  mutate(prop = value / sum(value)) %>%
  filter(new_property) %$%
  sum(prop)

prop_owner_occupier_housing_finance_commitments_to_new_dwellings <- 
  housing_finance_commitments %>%
  filter(Date == max(Date), !grepl("Total", Long_name), Series_Type == "Trend") %>% .[] %>%
  mutate(For_new_properties = grepl("(Constr.* of dwelling)|(new dwelling)", Long_name, ignore.case = TRUE)) %>% 
  mutate(prop = value / sum(as.numeric(value))) %>%
  filter(For_new_properties) %>% .[] %$%
  sum(prop)
  
@

In any case, general tax breaks like negative gearing are an inefficient way of supporting the rental market.\footnote{ 
  The most recent ABS lending data suggests that only \Sexpr{latex_percent(prop_investment_housing_finance_commitments_to_dwelling_construction)} of recent lending to investors is for constructed properties (ABS Cat. 5671.0 Table 8). One recent paper suggests the \Sexpr{latex_percent(prop_investment_housing_finance_commitments_to_dwelling_construction)} figure might be understated because the ABS data do not separately identify investor purchase of new housing (as opposed to construction of new housing). Assuming the share of lending for new dwellings by investors is a similar proportion as for owner occupiers, then \Sexpr{latex_percent(prop_owner_occupier_housing_finance_commitments_to_new_dwellings)} of lending for investment properties would be for new dwellings (either constructed or bought new). }   Blanket tax concessions for investors that primarily accrue to existing property owners through higher house prices are poorly targeted way of alleviating rental stress.  
...
cost_NG_limited_to <- function(Net_rent_amt.limit){
  ...
  target_sample_file %>%
    mutate(new_Net_rent_amt = pmaxC(Net_rent_amt, Net_rent_amt.limit),
           ...
           new_Tot_inc_amt = salary_income + if_else(Other_Income <= 0, 
...
number_affected <- function(Net_rent_amt.limit) {
  ...
  target_sample_file %>%
    mutate(new_Net_rent_amt = pmaxC(Net_rent_amt, Net_rent_amt.limit),
           ...
           new_Taxable_Income = pmaxC(new_Tot_inc_amt - Tot_ded_amt - NPP_loss_claimed - PP_loss_claimed, 0),
           prev_tax = income_tax(Taxable_Income, latest.sample.fy),
           post_tax = income_tax(new_Taxable_Income, latest.sample.fy),
           diff = post_tax - prev_tax) -> resultant_samplefile
  resultant_samplefile %$%
    # floating point
    sum((diff > 0.5) * WEIGHT)
...
cost_of_policy_carry_fwd <- function(Net_rent_amt.limit, h = 1L){
  ...
  sample_file_1314 %>%
    ...
    mutate(hasCG = runif(nrow(.)) < probCG,
           new_Taxable_Income = if_else(hasCG, 
...
<<HILDA-Number-of-properties, eval=TRUE, results='asis'>>=
hilda_w14_hh %>%
...
bind_rows("Primary" = primary_losses, "Non-Primary" = non_primary_losses, .id = "Type") %>% arrange(Occupation_description, desc(Type)) %T>%
  ...
  # Cosmetic:
  mutate(
    Occupation_description = if_else(grepl("Occupation not specified", Occupation_description), "Other", Occupation_description),
    Occupation_description = gsub("\\sworkers", "", Occupation_description, ignore.case = TRUE), 
    Occupation_description = if_else(grepl("Consultants", Occupation_description), "Consultants, apprentices", Occupation_description)
    ) %>%
...
@
\source{2\%\ sample file \Sexpr{latest.sample.fy}}
\end{table*}
...
The losses claimed are relatively small. In \Sexpr{latest.sample.fy} only \Sexpr{texNum(n_bizlosses)}~taxpayers claimed \Sexpr{texNum(-val_bizlosses, sig.figs = 1L, dollar = TRUE)} in business losses (compared to \Sexpr{texNum(n_rental_losses, sig.figs = 2L)} claiming \Sexpr{texNum(-val_rental_losses, sig.figs = 1L, dollar = TRUE)} in losses on property).
...
@
<<PP-losers-salary-comparison-evil-chart>>=
# JD requested
sample_file_1314 %>%
  merge(occupation_decoder, by = "Occ_code") %>%
  mutate(Sw_amt = as.double(Sw_amt)) %>%  # integer overflow on mean
  group_by(Occupation_description, Claiming_PP =  factor(if_else(Net_PP_BI_amt < 0, 
                                                                "Claiming PP losses", 
                                                                "Not claiming"), 
                                                         levels = c("Claiming PP losses", "Not claiming"))) %>%
  summarise(average_salary = mean(Sw_amt)) %>%
  group_by(Occupation_description) %>%
  mutate(salary_difference = if_else(Claiming_PP == "Not claiming", 
                                    average_salary, 
                                    first(average_salary, 
                                          order_by = Claiming_PP) - last(average_salary, 
                                                                         order_by = Claiming_PP))) %>%
  ungroup %>%
  filter(!grepl("Occupation not listed", Occupation_description)) %>%
  ...
  {
    grplot(., aes(x = Occupation_description, y = salary_difference, fill = Claiming_PP)) + 
...
<<PP-losers-salary-comparison-horiz-bar, cache=FALSE, fig.width=14, out.width="14in">>=
sample_file_1314 %>%
  merge(occupation_decoder, by = "Occ_code") %>%
  mutate(Sw_amt = as.double(Sw_amt)) %>%  # integer overflow on mean
  group_by(Occupation_description, Claiming_PP =  factor(if_else(Net_PP_BI_amt < 0, 
  ...
  mutate(salary_difference = if_else(Claiming_PP == "Not claiming", 
                                    ...
                                    first(average_salary, 
                                          order_by = Claiming_PP) - last(average_salary, 
                                                                         order_by = Claiming_PP)), 
         prop_claim_losses = first(n_individuals, order_by = Claiming_PP) / sum(n_individuals)) %>%
  ungroup %>%
  filter(!grepl("Occupation not listed", Occupation_description)) %>%
  ...
  mutate(Occupation_description = gsub("and type not specified or not listed", "", Occupation_description), 
         Occupation_description = gsub("and", "&", Occupation_description), 
         Occupation_description = if_else(grepl("Community.*Personal", Occupation_description), 
                                         "Community workers", 
                                         Occupation_description), 
         Occupation_description = if_else(grepl("^Clerica", Occupation_description), 
                                         "Clerical/Admin.", 
                                         Occupation_description)) %>%
  mutate(`Average salary` = average_salary) %>% 
  ...
  mutate(Occupation_description = factor(Occupation_description, 
                                         ...
                                         ordered = TRUE)) %T>%
                                         {
                                           # chart data for supply to pptx etc
                                           data <- .
                                           
                                           salaries <- 
                                             data %>% 
                                             select(Occupation_description, 
                                                    Claiming_PP, 
                                                    average_salary) %>% 
                                             spread(Claiming_PP, average_salary)
                                           prop_indiv_PP <-
                                             data %>% 
                                             select(Occupation_description, 
                                                           Claiming_PP, 
                                                           prop_claim_losses) %>% 
                                             spread(Claiming_PP, prop_claim_losses) 
                                           
                                           # salaries
                                           grattanCharts:::chart_data(data)
                                           
                                           # proportions are also desired
                                           ...
                                           if (!is.null(current_chunk_label)) {
                                             ...
                                             readr::write_csv(x = prop_indiv_PP, 
                                                              path = file.path(atlas, 
                                                                               paste0(current_chunk_label, 
                                                                                      "-prop_indiv",
                                                                                      ".csv")))
                                           }
...
sample_file %>%
  ...
  mutate(`Average salary` = average_salary) %>% 
  arrange(Claiming_PP, average_salary) %>%
  mutate(`Proportion claiming losses` = if_else(Claiming_PP == "Not claiming", 0, prop_claim_losses)) %>%
  mutate(Occupation_description = factor(Occupation_description, 
                                         levels = unique(.$Occupation_description), 
                                         ordered = TRUE)) %>%
                                         {
...
<<PP-losers-salary-comparison-ggrepel, fig.width=1.15*fig_width, out.width=paste0(1.15*out_width, "in")>>=
sample_file %>%
  merge(occupation_decoder, by = "Occ_code") %>%
  filter(!grepl("Occupation not listed", Occupation_description)) %>%
  mutate(Sw_amt = as.double(Sw_amt)) %>%  # integer overflow on mean
  group_by(Occupation_description) %>%
  summarise(average_salary_n_claiming = mean(Sw_amt[Net_PP_BI_amt >= 0]), 
            average_salary_claiming = mean(Sw_amt[Net_PP_BI_amt < 0]),
            prop_claiming = mean(Net_PP_BI_amt < 0)) %>%
  ungroup %>% 
  gather(Claiming, average_salary, average_salary_n_claiming:average_salary_claiming) %>%
  mutate(isClaiming = grepl("_n_claiming", Claiming),
         Claiming = factor(if_else(isClaiming, 
                                   "Not claiming", 
                                   "Claiming"), 
                           levels = c("Not claiming", 
                                      "Claiming"))) %>%
  # cosmetic
  mutate(Occupation_description = gsub("and type not specified or not listed", "", Occupation_description), 
         Occupation_description = gsub("and", "&", Occupation_description), 
         Occupation_description = if_else(grepl("Community.*Personal", Occupation_description), 
                                         "Community workers", 
                                         Occupation_description),
         Occupation_description = gsub("Administrative",
                                       "Admin.",
                                       Occupation_description),
         Occupation_description = sub(" workers", "", Occupation_description,
                                      ignore.case = TRUE),
         Occupation_description = if_else(grepl("apprenti", Occupation_description),
                                          "Consultants,\napprentices",
                                          Occupation_description)) %>%
  as.data.table %>%
  setkey(isClaiming, Occupation_description) %>%
  .[, c("xm", "ym") := list(average_salary, prop_claiming)] %>%
  .[.(TRUE, "Sales"), c("xm", "ym") := list(80e3, 0.05/100)] %>%
  .[.(FALSE, "Sales"), c("xm", "ym") := list(80e3, 0.05/100)] %>%
  .[] %>%
  {
    max.iter <- 12e3
    grplot(., aes(x = average_salary,
                  y = prop_claiming, 
      ...
      geom_point(size = 2,
                 show.legend = FALSE) + 
      geom_label_repel(family = if_knitting("helvet", ""),
                       ...
                       lineheight = 0.86,
                       # fontface = "bold", 
                       # point.padding = unit(0.05, 'npc'),
                       box.padding = unit(0.45, "lines"),
                       size = 16/(14/5), 
                       show.legend = FALSE) +
      # to force fill to appear
      geom_rect(xmin = NA, ymax = NA, xmax = NA, ymin = NA, na.rm = TRUE) +
      scale_x_continuous("Average salary",
                         label = grattan_dollar, 
                         limits = c(0.68*min(.$average_salary), 
                                    1.06*max(.$average_salary))) + 
      scale_y_continuous(labels = function(x) paste0(round(x * 100, 1), "%"),
                         limits = c(0, 0.5/100),
                         expand = c(0, 0)) + 
      guides(fill = guide_legend("PP losses", override.aes = list(fill = gpal(2))),
             color = FALSE) +
...
<<density-salary-by-PP-losses, fig.height=11, out.height="11in", dev=c("my_pdf", "png"), fig.ext = c("pdf", "png")>>=
sample_file %>%
  merge(occupation_decoder, by = "Occ_code") %>%
  filter(!grepl("Occupation not listed", Occupation_description)) %>%
  mutate(Sw_amt = as.double(Sw_amt), 
         Claiming_PP =  factor(if_else(Net_PP_BI_amt < 0, "Claiming PP losses", "Not claiming"), 
                               levels = c("Claiming PP losses", "Not claiming"))) %>%
  # cosmetic
  ...
  # order panels
  group_by(Occupation_description) %>%
  mutate(prop_claiming_PP = mean(Net_PP_BI_amt < 0)) %>%
  ungroup %>%
  arrange(prop_claiming_PP) %>%
  mutate(Occupation_description = factor(Occupation_description, 
                                         levels = unique(.$Occupation_description))) %>%
  mutate(Salary = Sw_amt) %>%
  grplot(aes(x = Salary, y = ..density.., fill = Claiming_PP)) + 
  geom_density(alpha = 0.8) +
  # geom_histogram(binwidth = 10e3) +
  scale_x_continuous(expand = c(0,0), label = grattan_dollar) + 
  # coord_cartesian(xlim = c(0, 250e3)) + 
  facet_grid(Occupation_description ~ ., scales = "free_y", switch = "y") + 
  theme(axis.text.y = element_blank(), 
        axis.ticks.y = element_blank(), 
        axis.line.y = element_blank(), 
        ...
        strip.text.y = element_text(angle = 180, 
                                    hjust = 1))
  @
\includegraphics[width=\columnwidth]{\Sexpr{atlas}/density-salary-by-PP-losses-1}
\end{figure}

\begin{figure}
\Caption{Relative salaries are higher for those earning PP}{Salary, \Sexpr{latest.sample.fy}}{fig:density-salary-by-PP-losses}
\includegraphics[width=\columnwidth]{\Sexpr{atlas}/density-salary-by-PP-losses-1}
\end{figure}
...
<<bizLosses-for-NPP>>=
n_bizlosses_NPP <- 
  sample_file[Net_NPP_BI_amt < 0,  sum(WEIGHT)]

avg_loss_claimed_NPP <- 
  sample_file[Net_NPP_BI_amt < 0,  mean(Net_NPP_BI_amt)]
avg_loss_claimed_NPP <- -1 * avg_loss_claimed_NPP
@
...
target_sample_file %>%
  selector(Net_rent_amt,
           ...
           WEIGHT,
           shallow = TRUE) %>%
  .[, quarantined_losses := pminC(Net_rent_amt, 0) + pminC(Net_farm_management_amt, 0) - Net_fincl_invstmt_lss_amt - NPP_loss_claimed - PP_loss_claimed] %>%
  .[, quarantined_income := Sw_amt + Alow_ben_amt + ETP_txbl_amt] %>%
  .[, Other_Income := Tot_inc_amt - quarantined_income - quarantined_losses] %>%
  .[, new_Other_Income := if_else(Other_Income <= 0, 
                                  Other_Income, 
  ...
  .[, old_tax := income_tax(Taxable_Income, latest.sample.fy)] %>%
  .[, new_tax := income_tax(new_Taxable_Income, latest.sample.fy)] %>%
  .[, tax_increase := WEIGHT * (new_tax - old_tax)] %>%
  .[, sum(tax_increase)]
@

We estimate that quarantining losses for passive investments would increase income tax collections by \Sexpr{texNum(cost_of_NG_only_year0, dollar=TRUE)} a year in the short term. Over the medium term, accrued losses -- losses not offset against recurrent investment income -- will be deducted from capital gains. Assuming no change in investor behaviour, the additional tax revenue would stabilise at around \Sexpr{texNum(cost_of_NG_only, dollar=TRUE)} a year.
...
% dev omit emf due to alpha
<<NG-vs-salary>>=
sample_file[Sw_amt %between% c(0, 250e3), .(Sw_amt, NG = as.double(Net_rent_amt < 0))] %>%
  setnames("Sw_amt", "Salary") %T>%
  chart_data %>%
  grplot(aes(x = Salary, y = NG)) + 
  geom_smooth(colour = DarkOrange, size = 1.5, method = "gam", formula = y ~ s(x, bs = 'bs')) +
  scale_y_continuous(label = function(x) paste0(round(100 * x), "%"), 
                     expand = c(0, 0)) + 
...
long_term = 10L
data.table(Threshold = c(seq(0, -20e3, by = -5e3), -50e3), -Inf) %>%
  .[, .(Costing = grattan_dollar(round(cost_NG_limited_to(.BY[[1]]))),
        `Number affected` = comma(round(number_affected(.BY[[1]]))), 
        `\\%` = latex_percent(number_affected(.BY[[1]]) / 
                                (target_sample_file[Gross_rent_amt > 0, sum(WEIGHT)]),
                              percentage_symbol = "\\%")),
    keyby = 'Threshold'] %>%
  .[, Threshold := paste0("$-$\\$", if_else(is.infinite(Threshold), "$\\infty$", comma(abs(Threshold))))] %>%
  .[, Costing := sub("$", "\\$", Costing, fixed = TRUE)] %>%
  selector(cols = c("Threshold", 'Costing', "Number affected", "\\%")) %>%
  # warns about not recognizing siuntix align operator
  {suppressWarnings(xtable(., align = "rrrrR"))} %>% 
  print.xtable(include.rownames = FALSE,
               booktabs = TRUE,
               floating = FALSE,
               align = "rrrrR", 
...
prop_landlords_geq_n_properties <- function(n) {
  ...
  if (hilda_used && exists("hilda_w14_hh")){
    hilda_w14_hh %>%
...
In addition to raising more than \Sexpr{texNum(total_cost_of_proposed_policy, dollar = TRUE)} a year, these changes will reduce (although not eliminate) distortions in investment choices toward debt financing of investment. Effective tax rates will increase a bit more for highly leveraged investors because they lose some of the tax benefits of upfront loss write offs \TBD{(Figure 18)}. However, effective tax rates will remain lower for this group because they still eventually write off their losses at a tax rate higher than they pay on gains. 

\chapter{Transition arrangements}
<<HILDA-duration-of-NG>>=
# (unique(hilda_all_waves_rp$oifrntp))
# [1] [-1] Not asked           [2] Loss                 [3] Break-even           [1] Profit              
# [5] [-3] Dont know           [-4] Refused/Not stated 
# 13 Levels: [-10] Non-responding person  [-9] Non-responding household  [-8] No SCQ  ... [3] Break-even 
if (hilda_used) {
  hilda_all_waves_rp <-
    lapply(1:15, function(y) {
      Year <- 2000L + y
      W <- letters[y]
      Data <- as.data.table(getExportedValue("hildaData", paste0("Combined_", W, "150c")))
      the_cols <- c("xwaveid", paste0(W, c("oifrntp", "lnwte")))
      
      if (!all(hasName(Data, the_cols))) {
        if (Year == 2001) {
          return(data.table())
        } 
        cat(Year)
      ...
      o <- 
        selector(Data,
                 ...
                 shallow = TRUE)
      o[, Year := 0L]
      set(o, j = "Year", value = Year)
      setnames(o, paste0(W, "oifrntp"), "FY_rental_property_profit_or_loss")
      setnames(o, paste0(W, "lnwte"), "WEIGHT")
      gc(full = TRUE, verbose = FALSE)
      o
    }) %>%
    rbindlist(use.names = TRUE, fill = TRUE)
}
# Which RPs have a loss followed by a Profit
if (!hilda_used && file.exists("tsv/xwaveid_with_NG_then_profit.tsv")) {
  xwaveid_with_NG_then_profit <- fread("tsv/xwaveid_with_NG_then_profit.tsv", 
                                       sep = "\t",
                                       na.strings = "NA")
} else {
  
  xwaveid_with_NG_then_profit <-
    hilda_all_waves_rp %>%
    .[, Profit_loss := if_else(grepl("Loss", FY_rental_property_profit_or_loss, fixed = TRUE), 
                               "Loss", 
                               if_else(grepl("Profit", FY_rental_property_profit_or_loss, fixed = TRUE),
                                       "Profit", 
                                       if_else(grepl("Break.even", FY_rental_property_profit_or_loss), 
                                               "Break even", 
                                               NA_character_)))] %>%
    setkey(xwaveid, Year) %>%
    .[, filled_down := na.locf(Profit_loss, na.rm = FALSE), keyby = "xwaveid"] %>%
    .[, loss := filled_down == "Loss"] %>%
    .[, next_Profit := shift(filled_down, type = "lead", fill = "Loss"), keyby = "xwaveid"] %>%
    .[, nextIsProfit := next_Profit %in% c("Break even", "Profit")] %>%
    .[, rw := any(and(loss, nextIsProfit)), 
      keyby = "xwaveid"] %>%
    .[(rw)] %T>%
    fwrite("tsv/xwaveid_with_NG_then_profit.tsv", 
           sep = "\t", 
           na = "NA") %>%
    .[]
}

if (!hilda_used && file.exists("tsv/hilda_rp_who_negatively_gearing.tsv")) {
  hilda_rp_who_negatively_gearing <- fread("tsv/hilda_rp_who_negatively_gearing.tsv", 
                                           sep = "\t",
                                           na.strings = "NA")
} else {
  
  hilda_rp_who_negatively_gearing <-
    xwaveid_with_NG_then_profit %>%
    .[, 
      # Returns NA by design
      loss_event := Profit_loss %in% c("Profit", "Break even") & shift(Profit_loss, type = "lead") == "Loss"] %>%
    .[, fill_down := na.locf(Profit_loss, na.rm = FALSE)] %>%
    .[, fill_up   := na.locf(Profit_loss, na.rm = FALSE, fromLast = TRUE)] %>%
    .[, left_censored := if_else(Year == min(Year), 
                                 TRUE, 
                                 coalesce(first(fill_up, order_by = Year) == "Loss",
                                          FALSE))] %>%
    .[, right_censored := if_else(Year == max(Year), 
                                  TRUE, 
                                  if_else(!is.na(last(fill_down, order_by = Year)), 
                                          last(fill_down, order_by = Year) == "Loss",
                                          is.na(last(fill_down, order_by = Year))))] %>% 
    .[] %T>%
    fwrite("tsv/hilda_rp_who_negatively_gearing.tsv", 
           sep = "\t",
           na = "NA") %>%
    .[]
  #0 = right censored
  ...
  #2 = left censored
  #3 = interval censored  
  #  optimistic: Loss NA NA Profit ==> Loss    Loss   Loss   Profit
  # pessimistic: Loss NA NA Proift ==> Loss    Profit Profit Profit
  # event_interval_optimistic = if_else(loss_event,
  #                                    1, 
  #                                    
  # )
  # Survival analysis
...
weights_by_waveid <- 
  hilda_rp_who_negatively_gearing %>% 
  selector(xwaveid, Year, WEIGHT) %>% 
  .[!is.na(WEIGHT)] %>%
  setkey(xwaveid, Year) %>%
  .[, .(lon_weight = last(WEIGHT)), keyby = .(xwaveid)] %>%
  .[]
...
ggsurv <- function(s, CI = 'def', plot.cens = T, surv.col = 'gg.def',
  ...
  library(ggplot2)
  strata <- if_else(is.null(s$strata) ==T, 1, length(s$strata))
  ...
  ggsurv.s <- function(s, CI = 'def', plot.cens = T, surv.col = 'gg.def',
    ...
    col <- if_else(surv.col == 'gg.def', 'black', surv.col)
  ...
  ggsurv.m <- function(s, CI = 'def', plot.cens = T, surv.col = 'gg.def',
    ...
    pl <- grplot(dat, aes(x = time, y = surv, group = group)) +
      xlab(xlab) + ylab(ylab) + ggtitle(main) +
...
<<Survival-NG-analysis-hilda>>=
#  optimistic: Loss NA NA Profit ==> Loss    Loss   Loss   Profit
# pessimistic: Loss NA NA Proift ==> Loss    Profit Profit Profit

cumsum_reset <- function(x) {
  as.integer(x * ave(x, c(0, cumsum(diff(x) != 0)), FUN = seq_along))
}

truth_durations <- function(cond) {
  # c(TRUE, TRUE, FALSE, TRUE, FALSE, TRUE, TRUE)
  # ==> c(2, 1, 2)
  y <- cumsum_reset(cond)
  y[lead(y, default = 0) == 0 & y != 0]
}

mean_duration <- function(x) {
  mean(truth_durations(x)) 
}

hilda_rp_who_negatively_gearing %>%
  .[, is_interval := is.na(Profit_loss) & !is.na(fill_up) & !is.na(fill_down), keyby = "xwaveid"] %>%
  .[, is_clear    := (is_interval & fill_up == fill_down) | !is_interval] %>%
  .[, optimistic  := if_else(!is_interval, 
                             Profit_loss,
                             if_else(is_clear,
                                     # i.e. Loss NA Loss ==> Loss Loss Loss
                                     fill_up, 
                                     "Loss"))] %>%
  .[, pessimistic := if_else(!is_interval, 
                             Profit_loss,
                             if_else(is_clear,
                                     # i.e. Loss NA Loss ==> Loss Loss Loss
                                     fill_up, 
                                     "Profit"))] %>%
  .[, gaps_filled := sum(!is_clear), keyby = "xwaveid"] %>%
  .[, CONDO := optimistic == "Loss" & !is.na(optimistic)] %>%
  .[, CONDP := pessimistic == "Loss" & !is.na(pessimistic)] %>%
  
  .[, years_with_losses_optimistic := cumsum_reset(optimistic == "Loss" & !is.na(optimistic)), keyby = "xwaveid"] %>%
  .[, years_with_losses_pessimistic := cumsum_reset(pessimistic == "Loss" & !is.na(pessimistic)), keyby = "xwaveid"] %>%
  .[, years_total := cumsum(!is.na(optimistic)), keyby = "xwaveid"] %>%
# mutate(entered_property_market = min(Year[which(Profit_loss == "Loss")])) %>%

  .[, .(
    # might be two spells could do min(optimistic) max(pessimistic) as well!
    optimistic1 = max(years_with_losses_optimistic) / max(years_total), 
    pessimistic1 = max(years_with_losses_pessimistic) / max(years_total),
    
    optimistic_avg_spell = mean_duration(CONDO),
    pessimistic_avg_spell = mean_duration(CONDP),
    
    years_at_risk = max(years_total), 
    
    censor_status = if_else(any(right_censored), 1, 2)),
    keyby = "xwaveid"] %>%
  .[]  %T>% 
  {
    hilda_surv_data <<- .
  } %>%
  .[, .(hazard_rate_optimistic = optimistic_avg_spell / years_at_risk, 
        hazard_rate_pessimistic = pessimistic_avg_spell / years_at_risk),
    keyby = "xwaveid"] %>%
  merge(weights_by_waveid) %$%
  weighted.mean(hazard_rate_pessimistic, lon_weight)
  
@
\begin{figure}
\Caption{Negative gearing spells are short}{Survival curve of negative gearing responding persons}{fig:NG-spells-are-short}
<<NG-survival-curve, dev="my_pdf", fig.ext="pdf">>=
hilda_surv_data_weights <- 
  merge(hilda_surv_data, weights_by_waveid)

hilda_survo <- survfit(Surv(optimistic_avg_spell, censor_status) ~ 1, 
                       data = hilda_surv_data_weights,
                       weights = lon_weight)
hilda_survp <- survfit(Surv(pessimistic_avg_spell, censor_status) ~ 1,
                       data = hilda_surv_data_weights, 
                       weights = lon_weight)

list(
  "high" = data.table(years = hilda_survp$time,
                      survival = hilda_survp$surv, 
                      survival_upper = hilda_survp$upper,
                      survival_lower = hilda_survp$lower),
   "low" = data.table(years = hilda_survo$time, 
                      survival = hilda_survo$surv,
                      survival_upper = hilda_survo$upper,
                      survival_lower = hilda_survo$lower), 
   "low" = data.table(years = 0, survival = 1, survival_upper = 1, survival_lower = 1), 
   "high" = data.table(years = 0, survival = 1, survival_upper = 1, survival_lower = 1)
) %>% 
  rbindlist(idcol = "imputer") %>%
  grplot(aes(x = years,
             y = survival,
             ymin = survival_lower,
             ymax = survival_upper, 
  ...
  scale_x_continuous(breaks = c(0, 1, 2, 5, 10, 15), limits = c(0, 16), expand = c(0, 0))
@
\includegraphics[width=\columnwidth]{\Sexpr{atlas}/NG-survival-curve-1}
...
<<haley-box>>=
haley_boxer <- function(initial_house_value = 700e3, 
                        holding_period = 10, 
                        final_value = 1e6, 
                        net_rent_return = 0.05, 
                        marginal_rate = 0.49,
                        cgt_discount = 0.50){
  
  cap_gain <- final_value - initial_house_value
  income_final <- final_value * net_rent_return
  CGT <- cap_gain * (1 - cgt_discount) * marginal_rate
  net_gain <- cap_gain - CGT
  
  list(
    initial_house_value = initial_house_value,
    holding_period = holding_period,
    final_value = final_value,
    net_rent_return = net_rent_return,
    cap_gain = cap_gain,
    ...
    net_gain = net_gain,
    value_sale = initial_house_value + net_gain,
    hurdle_rate = income_final / (initial_house_value + net_gain)
  )
}

haley_box <- haley_boxer()
@

Suppose Hayley, an investor in the top tax bracket purchases a house for \Sexpr{latex_dollar(haley_box$initial_house_value)} and holds it for \Sexpr{haley_box$holding_period} years. During that time the market price of the house increases to \Sexpr{latex_dollar(haley_box$final_value)}. She makes a net rental return of \Sexpr{latex_percent(haley_box$net_rent_return)} a year, so her  final income is \Sexpr{latex_dollar(haley_box$income_final)}. 

If she were to sell the house she would crystallise the \Sexpr{latex_dollar(haley_box$cap_gain)} in gains, paying tax on \Sexpr{latex_percent(haley_box$CGT_discount)} of the gains at her marginal tax rate of \Sexpr{latex_percent(haley_box$marginal_rate)} (\Sexpr{latex_dollar(haley_box$CGT)}). This would leave her with around \Sexpr{latex_dollar(haley_box$value_sale)} from the sale: \Sexpr{latex_dollar(haley_box$net_gain)} in net gains and her initial investment of \Sexpr{latex_dollar(haley_box$initial_house_value)}. 

The sale will only be worthwhile if Hayley can find an alternative property that yields more than her current rental income of \Sexpr{latex_dollar(haley_box$income_final)}  with the same opportunity for capital gains. With her \Sexpr{latex_dollar(haley_box$value_sale)} sale proceeds she would need to find a property with net rental return of more than \Sexpr{latex_percent(haley_box$hurdle_rate)} a year. 

Properties with rental yields below \Sexpr{latex_percent(haley_box$hurdle_rate)} but higher than her current \Sexpr{latex_percent(haley_box$net_rent_return, percentage_symbol = "")} would not be attractive because realising the tax liability on her current property erodes her capital base for investment.  

The higher the tax rate on capital gains the less willing she will be to realise gains to pursue new investment opportunities. If capital gains were taxed in full, rather than with a 50 per cent discount, her hurdle rate for the new investment would be \Sexpr{latex_percent(haley_boxer(cgt_discount = 0)$hurdle_rate)}.
\end{smallbox}
...
<<percent1_01_TeX>>=
percent1_01_TeX <- function(x){
  if (is.numeric(x) && mean(between(x, 0, 1) > 0.5)){
    paste0(formatC(round(100*x, 1), 
                   format = "f", flag = "#", digits = 1, width = 3), "\\%")
  } else {
    x
...
@
<<digit_01_TeX>>=
digit1_01_TeX <- function(x){
  if (is.numeric(x) && between(x, 0, 1)) {
    formatC(round(100*x, 1), 
            format = "f", flag = "#", digits = 1, width = 3)
  } else {
    x
  }
...
EMTR_Appendix_Table <- 
data.table::CJ(
  capital_gain_return = c(0.10, 0.08, 0.06, 0.05, 0.04), 
  marginal_tax_rate = c(0.47, 0.39, 0.345),
  CGT_discount = c(0.50, 0.25, 0), 
  Indexed = c(TRUE, FALSE), 
  return_type = c("real", "excess")
) %>%
  .[(!(Indexed & CGT_discount != 0.00))] %>%  # if indexed, CGT is ignored, so just choose keep one CGT discount
  .[, I := .I] %>%
  .[, emtr := emtr_rental(return_type = return_type, 
                          CGT.discount = CGT_discount, 
                          marginal.tax.rate = marginal_tax_rate,
                          inflation = 0.025,
                          risk.free.rate = 0.045,
                          capital.gains = capital_gain_return, 
                          gearing_ratio = 0, 
                          cgt_indexed = Indexed),
    keyby = "I"] %>%
  .[order(-capital_gain_return)] %>%
  # cosmetics
  .[, capital_gain_return := paste0("Nominal annual capital gain__", 
                                    formatC(100*capital_gain_return, 
                                            flag = "#",
                                            format = "f",
                                            digits = 1),
                                    percentage_symbol = "\\%")] %>%
  .[, I := NULL] %>%
  as_tibble %>%
  spread(capital_gain_return, emtr) %>% 
  ungroup %>%
  arrange(marginal_tax_rate, desc(return_type), `Nominal annual capital gain__5.0\\%`) %>%
  mutate(`CGT treatment` = if_else(Indexed, "Indexed", 
                                   paste(latex_percent(CGT_discount,
                                                       digits = 0, 
                                                       percentage_symbol = "\\%"),
                                         "discount"))) %>%
  as.data.table %>%
  ...
  {grattanCharts:::chart_data(.)} %>%
  .[] %>%
  .[, lapply(.SD, digit1_01_TeX)] %>%
  .[, lapply(.SD, omit_repeated)] %>%
  setnames(old = grep("_[a-z]", names(.), value = TRUE), 
           new = sub("^([a-z])",
                     "\\U\\1\\E", perl = TRUE,
                     (gsub("_([a-z])", 
                           " \\1", 
                           grep("_[a-z]",
                                ...
                                value = TRUE))))) %>% 
  .[] %>%
  # mutate_each(funs(shade_cell), -`Marginal tax rate`, -`Return type`, -`CGT treatment`) %>%
  # Increase space between groups by putting a newline with -6pt height (-9pt) for smaller
  .[, "\\phantom{.}" := if_else(!is.na(`Marginal tax rate`) & `Marginal tax rate` != first(`Marginal tax rate`), 
                                "&\\phantom{.}\\\\[-6pt]", 
                                if_else(!is.na(`Return type`) & `Return type` != first(`Return type`), 
                                        "&\\phantom{.}\\\\[-9pt]",
                                        ""))] %>%
  selector(`\\phantom{.}`, 
           `Marginal tax rate`, `Return type`, `CGT treatment`,
           `Nominal annual capital gain__4.0\\%`, 
           `Nominal annual capital gain__5.0\\%`, 
           `Nominal annual capital gain__6.0\\%`, 
           `Nominal annual capital gain__8.0\\%`,
           `Nominal annual capital gain__10.0\\%`) 
@
...
<<emtr-long-appendix, results='asis'>>=
EMTR_Appendix_Table %>%
  copy %>%
  {suppressWarnings(print_2heading_xtable(., 
                                          sanitize.text.function = function(x) x, 
                                          tabular.environment = "tabular", 
                                          xtable.align = "lQlllrrrrr",
                                          floating = FALSE))}
...
\node (Coloured) {%
<<emtr-long-appendix-cellcolored, results='asis'>>=
shade_cell <- function(x) {
  color <- cut(as.numeric(x), breaks = c(-Inf, 100*(1:6)/6, Inf), labels = c(paste0("Color", 1:6), "black"))
  # black background -> white text
  if_else(as.numeric(x) > 100, 
          paste0("\\cellcolor{", color, "}{\\textcolor{white}{", x, "}}"),
          paste0("\\cellcolor{", color, "}{", x, "}"))
}
...
cols_to_shade <- setdiff(names(EMTR_Appendix_Table), 
                         c("\\phantom{.}", "Marginal tax rate", "Return type", "CGT treatment"))

EMTR_Appendix_Table %>%
  copy %>%
  .[, (cols_to_shade) := lapply(.SD, shade_cell), .SDcols = c(cols_to_shade)] %>%
  .[] %>%
  {suppressWarnings(print_2heading_xtable(., 
                                          sanitize.text.function = function(x) x, 
                                          tabular.environment = "tabular", 
...
<<on_travis_cleaning>>=
if (on_travis) {
  the_node_size <- function() {
    bit <- 8L * .Machine$sizeof.pointer
    if (!(bit == 32L || bit == 64L)) {
      message("Unknown architecture", call. = FALSE)
      -1L
    }
    if (bit == 32L) 
      28L
  ...
  message("\tMemory used:\t", as.character(round(sum(gc()[, 1] * c(the_node_size(), 8)) / 1000^2), 1), " MB\n")
  message("\n\n\tRemoving objects...\n\n")
  rm(sample_files_all)
  rm(sample_file)
  rm(target_sample_file)
  rm(policy)
  rm(list = ls(pattern = "^policy"))
...
SessionInfo$packages %>%
  as.data.frame %>%
  as.data.table %>%
  selector(package, attached, date, source, md5ok) %>%
  .[, "att" := if_else(attached, "*", "")] %>%
  .[, attached := NULL] %>%
  xtable(caption = "Packages") %>% 
...
<<the_attached_packages, echo=TRUE>>=
c("bindrcpp", "hildaData", "hutils", "ggrepel", "testthat", "taxstats", 
...
"dplyr", "haven", "hutilscpp", "grattanCharts", "knitr", 
"data.table", "sessioninfo", "stats", "graphics", "grDevices", 
...
hutils::provide.dir("tex")
package_cite_input <- paste0("\\nocite{", paste0(paste0("R-", citPkgs), collapse=","), "}")
write(package_cite_input, file = "tex/package_cite_input.tex")
@
% citations
\input{tex/package_cite_input}
...
if(hilda_used || grepl("hilda", names(Filter(isTRUE, eapply(.GlobalEnv, is.data.frame))))){
  write("This paper uses unit record data from the Household, Income and Labour Dynamics in Australia (HILDA) Survey. The HILDA Project was initiated and is funded by the Australian Government Department of Social Services (DSS) and is managed by the Melbourne Institute of Applied Economic and Social Research (Melbourne Institute). The findings and views reported in this paper, however, are those of the authors and should not be attributed to either DSS or the Melbourne Institute.", 
        # In Acknowledgements.tex
        file = "tex/HILDA-acknowledgement-paragraph.tex")
}
@
