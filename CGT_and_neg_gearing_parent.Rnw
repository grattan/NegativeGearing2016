...
<<PP-losers-salary-comparison-ggrepel>>=
sample_file %>%
  merge(occupation_decoder, by = "Occ_code") %>%
  filter(!grepl("Occupation not listed", Occupation_description)) %>%
  mutate(Sw_amt = as.double(Sw_amt)) %>%  # integer overflow on mean
  ...
  mutate(Claiming = factor(if_else(grepl("_n_claiming", Claiming), 
                                  "Not claiming PP losses", 
                                  "Claiming PP losses"), 
                           levels = c("Not claiming PP losses", 
                                      "Claiming PP losses"))) %>%
  # cosmetic
  mutate(Occupation_description = gsub("and type not specified or not listed", "", Occupation_description), 
         Occupation_description = gsub("and", "&", Occupation_description), 
  ...
  {
    max.iter <- 20e3
    grplot(., aes(x = `Average salary`, y = prop_claiming, 
                  label = Occupation_description, colour = Claiming)) + 
      geom_point(size = 2) + 
      geom_text_repel(data = filter(., grepl("Not claiming", Claiming)), 
                      nudge_y = mean(.$prop_claiming)/8, 
                      nudge_x = -mean(.$average_salary)/8,
                      fontface = "bold", 
      ...
      geom_text_repel(data = filter(., !grepl("Not claiming", Claiming)), 
                      ...
                      max.iter = max.iter) + 
      scale_x_continuous(label = grattan_dollar, limits = c(0.8*min(.$average_salary), 
                                                            1.2*max(.$average_salary))) + 
      scale_y_continuous(label = percent, limits = c(0.8*min(.$prop_claiming), 1.2*max(.$prop_claiming))) + 
