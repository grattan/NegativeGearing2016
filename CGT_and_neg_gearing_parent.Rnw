...
rental_losses <- 
  ...
  ungroup %>%
  mutate(real_tot_avg_loss = cpi_inflator(tot_avg_loss, from_fy = fy.year, to_fy = "2013-14"))
...
NG_tax_exps.real <- 
  NG_tax_exps %>%
  ungroup %>% 
  mutate(avg.diff.real = cpi_inflator(avg.diff, from_fy = fy.year, to_fy = "2013-14")) %$%
  mean(avg.diff.real)
...
% dev omit emf due to alpha
<<NG-vs-salary>>=
sample_file[Sw_amt %between% c(0, 250e3), .(Sw_amt, NG = Net_rent_amt < 0)] %>%
