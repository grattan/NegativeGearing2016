...
sample_file %>%
  ...
  mutate(Claiming = factor(if_else(grepl("_n_claiming", Claiming), 
                                  "Not claiming", 
                                  "Claiming"), 
...
data.table(Threshold = c(seq(0, -20e3, by = -5e3), -50e3), -Inf) %>%
  .[, .(Costing = grattan_dollar(round(cost_NG_limited_to(.BY[[1]]))),
        ...
        `\\%` = latex_percent(number_affected(.BY[[1]]) / (sample_file_1314[, sum(Gross_rent_amt > 0)] * 50),
                              percentage_symbol = "\\%")),
    keyby = 'Threshold'] %>%
